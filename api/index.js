...
const createPennylaneInvoice = async (customerId, invoiceNumber, montagesData, totalHT, deadline) => {
   ...
   const invoiceLines = montagesData.map(m => {
    // 1. On cherche le prix sous toutes ses appellations possibles
    // Vérifie 'price', 'prix', 'montant', ou 'tarif'
    const unitPrice = m.total || m.price || 0;
    
    // 2. On sécurise les détails (tableau ou texte)
    let detailsText = '';
    if (m.details && Array.isArray(m.details)) {
        detailsText = m.details.join(', ');
    } else if (m.glassType && Array.isArray(m.glassType)) {
        detailsText = m.glassType.join(', ');
    } else if (typeof m.details === 'string') {
        detailsText = m.details;
    }

    // 3. Log de vérification pour vous aider dans les Logs Vercel
    console.log(`💰 Montage Ref: ${m.reference} | Prix détecté: ${unitPrice}`);

    return {
        label: detailsText ? `${m.reference || 'Montage'} — ${detailsText}` : `${m.reference || 'Montage'}`,
        quantity: 1,
    ...
    const data = await res.json();
    console.log("📄 Réponse Pennylane:", JSON.stringify(data));
    // L'API v2 retourne l'objet directement à la racine
    const invoice = data.customer_invoice || data;
    ...
    try {
        ...
        if (PENNYLANE_TOKEN && invoiceData && invoiceData.length > 0) {
            try {
                ...
                // 2. Créer la facture dans Pennylane (finalisée directement)
                ...
                // 3. Polling PDF — on attend que Pennylane génère le PDF (max 60s)
                // Vercel Hobby = 60s max par requête, on utilise ce budget ici
                ...
                for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
                    ...
                    if (pdfRes.ok) {
                        ...
                        console.log(`✅ PDF prêt (${Math.round(pdfBuffer.byteLength / 1024)} Ko) après ${attempt} tentative(s)`);
                        break;
                    }
                    console.log(`⏳ PDF pas encore prêt (${pdfRes.status}), attente ${INTERVAL_MS/1000}s...`);
                    await new Promise(r => setTimeout(r, INTERVAL_MS));
                }

                if (!pdfBuffer) {
                    throw new Error(`PDF non disponible après ${MAX_ATTEMPTS} tentatives — email non envoyé`);
                }

                // 4. Envoyer le mail avec le PDF en pièce jointe
                const emailBodyHtml = `<p>Bonjour <strong>${user.nomSociete}</strong>,</p>
                ...
                await resend.emails.send({
                    ...
                    html: getEmailTemplate(`Facture ${invoiceNumber}`, emailBodyHtml,
