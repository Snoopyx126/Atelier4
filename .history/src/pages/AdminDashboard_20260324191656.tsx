...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose, onInvoicePublished }) => {
    ...
    const handlePublishAndSend = async () => {
        ...
        const toastId = toast.loading("Génération du numéro et du document...");

        try {
            // --- ÉTAPE 1 : GÉNÉRATION DU NUMÉRO UNIQUE ---
            const nomsMois = [
                "Janvier", "Février", "Mars", "Avril", "Mai", "Juin",
                "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"
            ];
            const moisActuel = nomsMois[new Date().getMonth()];
            const anneeActuelle = new Date().getFullYear();
            
            // Format : "FACTURE MARS 2026 - NOM DU MAGASIN"
            const invoiceNumber = `Facture ${moisActuel} ${anneeActuelle} - ${client.nomSociete}`.toUpperCase();
            // ----------------------------------------------
            ...
            const imgData = canvas.toDataURL('image/jpeg', 0.3); // Compression pour Vercel
            ...
            const payload = { 
                ...
                pdfUrl: '#', 
                sendEmail: true, 
                // Sécurité Vercel : null si plus de 20 montages
            ...
            if (data.success) { 
                ...
                // Si erreur de doublon, on affiche le message précis
                toast.error(data.message || "Erreur d'enregistrement", { id: toastId });
            }
        } catch (error) { 
            console.error(error);
