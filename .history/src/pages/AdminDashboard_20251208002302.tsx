...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose }) => {
    ...
    // Fix: Afficher tous les montages Terminés (sans filtre de date stricte)
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl bg-white p-8 print:p-0">
                <div className="print:hidden"><DialogHeader><DialogTitle className="text-3xl font-bold text-gray-900 flex items-center gap-2"><Receipt className="w-6 h-6" /> Facture Provisoire ({monthlyMontages.length} montages)</DialogTitle></DialogHeader></div>
  ...
  const handleStatusChange = async (id: string, newStatus: string) => { 
      ...
      try {
          ...
          if (data.success) {
               ...
               // Revert local change and notify
