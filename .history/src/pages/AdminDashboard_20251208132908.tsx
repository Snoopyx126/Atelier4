...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose, onInvoicePublished }) => {
    ...
    const handlePublishAndSend = async () => {
        ...
        try {
            // 1. Capture PDF optimisée (Qualité/Poids)
...
const ClientInvoicesModal: React.FC<{ client: Client | null; invoices: FactureData[]; isOpen: boolean; onClose: () => void; onDelete: (id: string) => void; }> = ({ client, invoices, isOpen, onClose, onDelete }) => {
            ...
            <DialogContent className="max-w-2xl bg-white p-6 max-h-[90vh] overflow-y-auto">
                ...
                {invoices.length === 0 ? <div className="text-center py-10 text-gray-500">Aucune facture.</div> : (
                    <div className="space-y-3 pt-4">
                        {invoices.map(inv => (
                            <Card key={inv.id} className="flex justify-between items-center p-3">
                                <div><p className="font-semibold">{inv.invoiceNumber}</p><p className="text-xs text-gray-500">{new Date(inv.dateEmission).toLocaleDateString()}</p></div>
                                <div className="flex items-center gap-3">
                                    <span className="font-bold">{inv.totalTTC.toFixed(2)} €</span>
                                    <Button size="sm" variant="destructive" onClick={() => { if(confirm("Supprimer ?")) onDelete(inv.id); }}><Trash2 className="w-4 h-4" /></Button>
                                </div>
                            </Card>
                        ))}
                    </div>
...
// --- DASHBOARD PRINCIPAL (VOTRE DESIGN D'ORIGINE) ---
  ...
  return (
    <div className="min-h-screen bg-gray-100/50 flex flex-col">
      ...
      <div className="flex-grow pt-24 pb-10 px-6 container mx-auto max-w-7xl">
        ...
        {/* TABS (REMISE EN PLACE DES ACCORDÉONS) */}
