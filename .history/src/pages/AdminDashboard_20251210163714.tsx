...
const groupedByMonthAndShop = filteredMontages.reduce((acc: any, m) => { 
    // Tentative de trouver le nom fiable dans la liste 'clients'
    const clientFound = clients.find(c => c._id === m.userId);
    // Utiliser le nom trouvé ou le nom du montage, ou "Client Inconnu" comme clé
    const clientNameKey = clientFound?.nomSociete || m.clientName || 'Client Inconnu (ID: ' + m.userId.substring(0, 4) + ')';
    
    const monthName = new Date(m.dateReception).toLocaleDateString('fr-FR', {month:'long', year:'numeric'}); 
    
    if(!acc[monthName]) acc[monthName] = {}; 
    
    // On groupe par le nom fiable (clientNameKey)
