...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("1. Soumission du formulaire lancée."); // <-- DEBUG
    console.log("E-mail tenté:", formData.email); // <-- DEBUG
    ...
    try {
      ...
      console.log("2. Réponse du serveur reçue. Statut:", response.status); // <-- DEBUG
      const data = await response.json();

      if (!response.ok) {
        // Le serveur a retourné une erreur (401, 403, 500...)
        console.error("3. Échec de la connexion. Message serveur:", data.message); // <-- DEBUG
        throw new Error(data.message || `Erreur de connexion (Statut: ${response.status}).`); 
      }

      // 4. Succès de la connexion
      console.log("4. Connexion réussie ! Redirection..."); // <-- DEBUG
      ...
      console.error("5. Erreur capturée dans le bloc catch:", error.message); // <-- DEBUG
