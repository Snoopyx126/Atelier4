...
// --- INTERFACES ---
interface UserData { 
    id: string; nomSociete: string; role?: string; assignedShops?: any[]; pricingTier?: 1 | 2; 
}
interface Montage { 
...
// --- UTILITAIRES (Sortis du composant pour éviter les bugs) ---
const normalize = (text: string | undefined): string => {
    if (!text) return "";
    return text.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
};

const CATEGORY_COSTS: Record<string, { 1: number, 2: number }> = { 'Cerclé': { 1: 7.00, 2: 3.60 }, 'Percé': { 1: 15.90, 2: 12.00 }, 'Nylor': { 1: 14.90, 2: 12.00 } };
const GLASS_COSTS: Record<string, { 1: number, 2: number }> = { 'Verre 4 saisons': { 1: 28.80, 2: 28.80 }, 'Verre Dégradé': { 1: 50.00, 2: 48.00 }, 'Verre de stock': { 1: 0.00, 2: 0.00 } };
const DIAMONDCUT_COSTS: Record<string, { 1: number, 2: number }> = { 'Facette Lisse': { 1: 39.80, 2: 21.50 }, 'Facette Twinkle': { 1: 79.80, 2: 60.00 }, 'Diamond Ice': { 1: 93.60, 2: 60.00 }, 'Standard': { 1: 0.00, 2: 0.00 } };
...
const MontageCard = ({ m, tier, onPhoto, onEdit, onDelete, isManager, fileInputRefs, handleUpload }: any) => {
    ...
    // Client : Peut modifier/supprimer SEULEMENT si "En attente" ou "Reçu".
        ...
        <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-white rounded-lg border-l-4 border-gray-100 gap-4 hover:shadow-md ${statusColors[m.statut] || 'border-gray-300'}`}>
            <div className="flex-1">
                ...
                <div className="mb-2 flex gap-2 items-center">
                     ...
                     {m.createdBy && m.createdBy.includes("Manager") && <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded border border-indigo-100 font-medium">Ajout Responsable</span>}
                </div>
                <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    {m.glassType?.map(g => <Badge key={g} className="bg-green-100 text-green-800">{g.replace('Verre ', '')}</Badge>)}
                </div>
...
const MesCommandes = () => {
  ...
  const [formData, setFormData] = useState({
      ref: "", frame: "", category: "Cerclé", glassType: [] as string[], 
      urgency: "Standard", diamondCut: "Standard", engraving: 0, shapeChange: false, desc: "", shopId: ""
  });
  ...
  const fetchData = async (user: UserData) => {
  ...
  const handleSave = async (e: React.FormEvent) => { 
      ...
      const payload = { 
          ...
          reference: formData.ref, frame: formData.frame, description: formData.desc, category: formData.category, 
          glassType: formData.glassType, urgency: formData.urgency, diamondCutType: formData.diamondCut, 
          engravingCount: formData.engraving, shapeChange: formData.shapeChange, 
      ...
      try { 
          ...
          const data = await res.json(); 
          if (data.success) { 
              toast.success("Enregistré !"); setIsDialogOpen(false); fetchData(userData); 
  ...
  const handleDelete = async (id: string) => { 
      ...
      await fetch(`${getApiUrl()}/api/montages/${id}`, { method: 'DELETE' }); 
      fetchData(userData!); toast.success("Supprimé"); 
  };

  const handlePhotoLoad = async (id: string) => {
      toast.loading("Chargement...", { id: 'load' });
      try {
          const res = await fetch(`${getApiUrl()}/api/montages/${id}`);
          const data = await res.json();
          if(data.success && data.montage.photoUrl) { setSelectedPhotoUrl(data.montage.photoUrl); toast.dismiss('load'); }
          else toast.error("Introuvable", { id: 'load' });
      } catch(e) { toast.error("Erreur", { id: 'load' }); }
  };

  const handlePhotoUpload = async (id: string, file: File | undefined) => {
      if (!file) return;
      const form = new FormData(); form.append('photo', file);
      toast.loading("Envoi...", { id: 'up' });
      await fetch(`${getApiUrl()}/api/montages/${id}/photo`, { method: 'POST', body: form });
      toast.success("Envoyé", { id: 'up' }); fetchData(userData!);
  };

  const openForm = (m?: Montage) => {
      if (m) {
          setEditingId(m._id);
          setFormData({ ref: m.reference||"", frame: m.frame||"", category: m.category||"Cerclé", glassType: m.glassType||[], urgency: m.urgency||"Standard", diamondCut: m.diamondCutType||"Standard", engraving: m.engravingCount||0, shapeChange: m.shapeChange||false, desc: m.description||"", shopId: m.userId });
      } else {
          setEditingId(null);
          setFormData({ ref: "", frame: "", category: "Cerclé", glassType: [], urgency: "Standard", diamondCut: "Standard", engraving: 0, shapeChange: false, desc: "", shopId: userData?.assignedShops?.[0]?._id || "" });
      }
  ...
  const filtered = montages.filter(m => {
      ...
      return (normalize(m.reference).includes(s) || normalize(m.frame).includes(s) || normalize(m.clientName).includes(s));
  });
  ...
  const grouped = filtered.reduce((acc: any, m) => {
      ...
      if (!acc[dateKey][shopKey]) acc[dateKey][shopKey] = [];
      acc[dateKey][shopKey].push(m);
      return acc;
  }, {});
      ...
      <div className="flex-grow pt-24 pb-10 px-4 md:px-8 container mx-auto max-w-7xl">
        <div className="flex justify-between items-center mb-8">
            <div>
                ...
                <p className="text-gray-500">{userData.role === 'manager' ? "Vue Responsable : Suivi des boutiques" : "Suivi de vos montages"}</p>
            </div>
                ...
                <TabsContent key={tab} value={tab}>
                    <Card className="border-0 shadow-md"><CardContent className="p-6 bg-gray-50/50 min-h-[400px]">
                        {Object.entries(grouped).map(([month, shops]: any) => (
                            <Accordion key={month} type="multiple" className="mb-4 bg-white rounded-lg shadow-sm border">
                                <AccordionItem value={month} className="border-0">
                                    <AccordionTrigger className="px-4 hover:no-underline bg-gray-50 rounded-t-lg"><div className="flex gap-2 items-center"><Calendar className="w-5 h-5 text-blue-600"/><span className="capitalize font-bold text-lg">{month}</span></div></AccordionTrigger>
                                    <AccordionContent className="p-4 space-y-4">
                                        {Object.entries(shops).map(([shopName, items]: any) => {
                                            const tabItems = (items as Montage[]).filter(m => tab === 'historique' ? (m.statut === 'Terminé' || m.statut === 'Expédié') : (m.statut !== 'Terminé' && m.statut !== 'Expédié'));
                                            if (tabItems.length === 0) return null;
                                            
                                            // --- VUE MANAGER (GROUPE PAR MAGASIN) ---
                                            if (userData.role === 'manager') {
                                                return (
                                                    <div key={shopName} className="border rounded-lg p-4 bg-gray-50/30">
                                                        <div className="flex items-center gap-2 mb-3 border-b pb-2"><Store className="w-4 h-4 text-blue-600"/><h3 className="font-bold">{shopName}</h3><span className="text-xs text-gray-400">({tabItems.length})</span></div>
                                                        <div className="space-y-3">
                                                            {tabItems.map(m => <MontageCard key={m._id} m={m} tier={userData.pricingTier} isManager={true} onPhoto={handlePhotoLoad} onEdit={openForm} onDelete={handleDelete} fileInputRefs={fileInputRefs} handleUpload={handlePhotoUpload} />)}
                                                        </div>
                                                    </div>
                                                );
                                            }
                                            // --- VUE CLIENT (LISTE SIMPLE) ---
                                            return (
                                                <div key={shopName} className="space-y-3">
                                                    {tabItems.map(m => <MontageCard key={m._id} m={m} tier={userData.pricingTier} isManager={false} onPhoto={handlePhotoLoad} onEdit={openForm} onDelete={handleDelete} fileInputRefs={fileInputRefs} handleUpload={handlePhotoUpload} />)}
                                                </div>
                                            );
                                        })}
                                    </AccordionContent>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}><DialogContent className="max-w-2xl bg-white"><DialogHeader><DialogTitle>{editingId ? "Modifier" : "Nouveau"}</DialogTitle></DialogHeader>
            <form onSubmit={handleSave} className="space-y-4 pt-4">
                {userData.role === 'manager' && (
                    <div><Label>Magasin concerné</Label><Select value={formData.shopId} onValueChange={v => setFormData({...formData, shopId: v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{userData.assignedShops?.map((s:any) => <SelectItem key={s._id} value={s._id}>{s.nomSociete}</SelectItem>)}</SelectContent></Select></div>
                )}
                <div className="grid grid-cols-2 gap-4">
                    <div><Label>Référence</Label><Input value={formData.ref} onChange={e=>setFormData({...formData, ref: e.target.value})} required/></div>
                    <div><Label>Monture</Label><Input value={formData.frame} onChange={e=>setFormData({...formData, frame: e.target.value})} required/></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div><Label>Type</Label><Select value={formData.category} onValueChange={v => setFormData({...formData, category: v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Cerclé">Cerclé</SelectItem><SelectItem value="Percé">Percé</SelectItem><SelectItem value="Nylor">Nylor</SelectItem></SelectContent></Select></div>
                    <div><Label>Urgence</Label><Select value={formData.urgency} onValueChange={v => setFormData({...formData, urgency: v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Standard">Standard</SelectItem><SelectItem value="Express -24H">Express -24H</SelectItem></SelectContent></Select></div>
                </div>
