...
// Nous n'utilisons pas le composant Input custom pour éviter les conflits
...
const Profil = () => {
  ...
  // --- 3. Soumission (Logique JS Pure + Appel API) ---
  const handleSubmit = async () => {
    
    // Validation JavaScript (qui remplace la validation HTML)
    ...
    // Vérification de sécurité (Mot de passe actuel)
    ...
    try {
        ...
        // Appel à l'API (maintenant actif)
        ...
        if (!response.ok) {
            // Le serveur a renvoyé un statut d'erreur (4xx ou 5xx)
            throw new Error(data.message || "Échec de la mise à jour du profil.");
        }
        
        // Succès : Mise à jour du localStorage avec les données renvoyées par l'API
        ...
        alert(error.message || "Une erreur est survenue lors de la mise à jour du profil. Assurez-vous que votre backend est démarré.");
    } finally {
        setIsLoading(false);
    }
  };
        ...
        <div className="w-full max-w-lg space-y-8">
          ...
          <Card className="shadow-lg border-gray-200">
            ...
            <CardContent className="space-y-6">
              {/* Conteneur <div> à la place de <form> */}
              <div className="space-y-6"> 
                ...
                {/* 2. Email - INPUT NATIF (type text pour contourner les validations) */}
                <div>
                ...
                {/* 4. Mot de passe actuel - INPUT NATIF (Conditionnel) */}
