                ...
                <DialogContent className="DialogContent max-w-4xl bg-white p-0 overflow-hidden flex flex-col max-h-[95vh]">
                    ...
                    <div className="overflow-y-auto p-6 bg-gray-50 flex-grow">
                        <div id="invoice-content" className="p-10 bg-white text-black border shadow-sm mx-auto max-w-[210mm] min-h-[297mm] flex flex-col justify-between"> 
                            <div>
                                <div className="flex justify-between items-start mb-10"><div><h1 className="text-4xl font-extrabold text-gray-900 mb-2">Pro Forma</h1><p className="text-gray-500 font-medium">N° {invoiceNumber}</p><p className="text-sm text-gray-400">Date: {today.toLocaleDateString('fr-FR')}</p></div><div className="text-right"><h2 className="text-xl font-bold text-gray-800">{FACTURE_INFO.name}</h2><p className="text-sm text-gray-600">{FACTURE_INFO.address}</p><p className="text-sm text-gray-600">{FACTURE_INFO.zipCity}</p><p className="text-sm text-gray-600">SIRET: {FACTURE_INFO.siret}</p></div></div>
...
const AdminDashboard = () => {
  ...
  const handleExportCSV = () => {
    const headers = ["Date Reception", "Client", "Reference", "Monture", "Categorie", "Statut", "Prix HT"];
    const csvContent = [
      headers.join(";"),
      ...montages.map(m => {
        const client = clients.find(c => c._id === m.userId)?.nomSociete || "Inconnu";
  ...
  return (
      ...
      <div className="flex-grow pt-24 pb-10 px-6 container mx-auto max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex gap-3">
                {/* ✅ BOUTON AJOUTÉ POUR EXPORT CSV */}
                <Button variant="outline" onClick={handleExportCSV} className="bg-white gap-2"><FileText className="w-4 h-4" /> Export CSV</Button>
    ...
    <Card className="shadow-md border-0">
        <CardHeader className="bg-white border-b">
            <CardTitle>Fiches Clients & Facturation</CardTitle>
        </CardHeader>
            ...
            {filteredClients.map(c => (
                <div key={c._id} className="p-4 border-b last:border-0 flex flex-col md:flex-row justify-between items-start md:items-center bg-gray-50 hover:bg-gray-100 transition-colors gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {/* ✅ BOUTON VALIDER (N'apparait que si non validé) */}
                        {!c.isVerified && (
                            <Button 
                                size="sm" 
                                className="bg-black text-white hover:bg-gray-800"
                                onClick={async (e) => {
                                    e.stopPropagation(); // Empêche d'ouvrir les factures en cliquant ici
                                    if(confirm(`Valider le compte de ${c.nomSociete} ? Un email lui sera envoyé.`)) {
                                        try {
                                            const baseUrl = window.location.hostname === "localhost" ? "http://localhost:3000" : "https://atelier4.vercel.app";
                                            const res = await fetch(`${baseUrl}/api/users/${c._id}`, {
                                                method: 'PUT',
                                            ...
                                            const data = await res.json();
                                            if(data.success) {
                                                toast.success("Client validé et notifié !");
                                                // Mise à jour immédiate de l'affichage local
                                                setClients(prev => prev.map(client => client._id === c._id ? {...client, isVerified: true} : client));
                                            } else {
                            ...
                            >
                                <CheckCircle2 className="w-4 h-4 mr-2" /> Valider Compte
                            </Button>
                        )}
