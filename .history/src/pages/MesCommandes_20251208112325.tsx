...
// Interface simplifiée pour les factures (pour l'affichage client)
interface Facture {
    id: string;
...
const MesCommandes = () => {
  ...
  // Formulaire (simplifié pour la concision)
  ...
  const fetchFactures = async (userId: string) => {
      // Données de facture simulées en attendant le développement de la route API /api/factures
      setFactures([
          { id: 'FCT-202511-001', montageReference: 'MNT-4567', dateEmission: '2025-11-01', totalTTC: 124.80, pdfUrl: '#' },
          { id: 'FCT-202512-002', montageReference: 'MNT-4568', dateEmission: '2025-12-01', totalTTC: 239.90, pdfUrl: '#' },
      ]);
  };
    ...
    try {
        ...
        if (data.success) {
            ...
            fetchMontages(user.id); 
             ...
             toast.error(`Erreur création: ${data.message || 'Erreur inconnue.'}`);
        }
    } catch (error) { 
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-grow pt-24 pb-12 px-4 max-w-5xl mx-auto w-full">
        <div className="flex justify-between items-center mb-4">
            <h1 className="text-3xl font-bold text-gray-900">Suivi Client</h1>
            <Button variant="outline" onClick={() => navigate("/dashboardpro")}>← Retour Dashboard</Button>
        </div>
        ...
        <Card className="mb-8 p-4 shadow-sm border-l-4 border-blue-300">
            ...
            <div className="text-sm text-gray-600 mt-1">
                ...
                {user.address === "Adresse non renseignée" && (
                    <p className="text-xs text-red-500 mt-2">⚠️ Votre adresse de facturation n'est pas complète.</p>
                )}
            </div>
        </Card>
        ...
        <Tabs defaultValue={initialTab} className="space-y-6">
            <TabsList className="bg-white p-1 shadow-sm border h-auto w-full md:w-auto">
                <TabsTrigger value="commandes" className="px-6 py-2"><ShoppingCart className="w-4 h-4 mr-2" /> Commandes ({montages.length})</TabsTrigger>
            ...
            <TabsContent value="commandes" className="space-y-6">
                {/* ... (Contenu du formulaire d'ajout de montage) ... */}
            ...
            <TabsContent value="factures" className="space-y-6">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Factures disponibles</h2>
                
                {factures.length === 0 ? (
                    <p className="text-center py-10 text-gray-500">Aucune facture n'a encore été émise par l'atelier.</p>
                ) : (
                    <div className="space-y-3">
                        {factures.map(f => (
                            <Card key={f.id} className="p-4 flex justify-between items-center shadow-sm">
                                <div>
                                    <p className="font-bold text-lg text-gray-900">{f.id} - Réf. {f.montageReference}</p>
                                    <p className="text-sm text-gray-500">Émise le {new Date(f.dateEmission).toLocaleDateString('fr-FR')}</p>
                                </div>
                                <div className="flex items-center gap-4">
                                    <span className="text-xl font-extrabold text-black">{f.totalTTC.toFixed(2)} € TTC</span>
                                    <Button variant="outline" onClick={() => window.open(f.pdfUrl, '_blank')} className="bg-blue-600 hover:bg-blue-700 text-white">
