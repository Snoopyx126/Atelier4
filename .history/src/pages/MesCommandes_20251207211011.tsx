...
const MesCommandes = () => {
  ...
  const handleAddMontage = async (e: React.FormEvent) => {
    ...
    // Validation simple
    if (!reference || !frame) {
        toast.error("La référence et la monture sont obligatoires.");
        return;
    }
      ...
      <div className="flex-grow pt-24 pb-12 px-4 max-w-5xl mx-auto w-full">
            ...
            <form onSubmit={handleAddMontage} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input 
                            placeholder="Ex: Cartier Panthère Or" 
                            value={frame}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center space-x-2">
                                <Checkbox 
                                    id="gravure" 
                                    checked={options.includes("Gravure")}
                                    onCheckedChange={(checked) => handleOptionChange("Gravure", checked as boolean)}
                                />
                                <label htmlFor="gravure" className="text-sm font-medium leading-none cursor-pointer">
                                    Gravure
                                </label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <Checkbox 
                                    id="diamond" 
                                    checked={options.includes("Diamond Cut")}
                                    onCheckedChange={(checked) => handleOptionChange("Diamond Cut", checked as boolean)}
                                />
                                <label htmlFor="diamond" className="text-sm font-medium leading-none cursor-pointer">
                                    Diamond Cut
                                </label>
                            </div>
                        </div>
                ...
                {/* 3. Commentaire */}
                <div className="space-y-2">
                    <Label>Instructions Spéciales (Facultatif)</Label>
                    <Input 
                        placeholder="Précisions sur le verre, décentrement, etc." 
                        value={description}
                ...
                <Button type="submit" disabled={isSubmitting} className="w-full md:w-auto">
                    {isSubmitting ? "Envoi..." : "Valider le dossier"}
                </Button>
            </form>
            </CardContent>
        </Card>
        ...
        <div className="space-y-6">
            ...
            {Object.keys(groupedMontages).length === 0 ? (
                <p className="text-center text-gray-500">Aucun dossier pour le moment.</p>
            ) : (
                                        ...
                                        <div key={montage._id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-gray-50 rounded-md border border-gray-100 gap-4">
                                            <div>
                                                ...
                                                <div className="flex flex-wrap items-center gap-2 mb-2">
                                                    ...
                                                    {montage.options && montage.options.map(opt => (
                                                        <span key={opt} className="text-xs bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded-full border border-yellow-100">
                                                            {opt}
                                                        </span>
                                                    ))}
                                                </div>
                                                ...
                                                {montage.description && (
                                                    <p className="text-sm text-gray-500 italic mb-1">"{montage.description}"</p>
                                                )}
                                                
                                                <p className="text-xs text-gray-400">
                                                    Envoyé le {new Date(montage.dateReception).toLocaleDateString('fr-FR')}
                                                </p>
                                            </div>
