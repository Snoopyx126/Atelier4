      ...
      <div className="flex-grow pt-24 pb-10 px-6 container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 space-y-6">
            {/* Infos de Profil */}
            <Card className="shadow-lg border-l-4 border-l-black-500">
              <CardHeader>
                <CardTitle>Mon Profil</CardTitle>
                <CardDescription>Vos informations de contact et de facturation.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-2">
                <div className="flex items-center text-sm text-gray-700 gap-2"><Mail className="w-4 h-4 text-black-500" /> {user.email}</div>
                {user.phone && <div className="flex items-center text-sm text-gray-700 gap-2"><Phone className="w-4 h-4 text-blue-500" /> {user.phone}</div>}
                <div className="flex items-center text-sm text-gray-700 gap-2"><FileText className="w-4 h-4 text-black-500" /> SIRET: {user.siret}</div>
                ...
                <Button onClick={handleEditProfile} className="w-full bg-black-500 hover:bg-black-600 text-white mt-4">
                  Modifier mon profil
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-lg border-l-4 border-l-red-500">
              ...
              <CardContent className="space-y-4">
                <Link to="/comment-ca-marche" className="w-full block">
                  <Button className="w-full bg-black-500 hover:bg-black-600 text-white">
                    Comment ça marche ?
                  </Button>
                </Link>
                <Button onClick={handleLogout} className="w-full bg-black-500 hover:bg-black-600 text-white">
                  Déconnexion
                </Button>
              </CardContent>
            </Card>
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* Accès aux Commandes */}
            <Card 
              className="shadow-lg border-l-4 border-l-black hover:shadow-xl transition-shadow cursor-pointer" 
            ...
            {/* Accès aux Factures */}
            <Card 
              className="shadow-lg border-l-4 border-l-green-500 hover:shadow-xl transition-shadow cursor-pointer" 
