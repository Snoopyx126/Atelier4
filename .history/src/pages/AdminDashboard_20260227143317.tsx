...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose, onInvoicePublished }) => {
    ...
    try {
        ...
        // --- LOGIQUE MULTI-PAGES ---
        const pdfWidth = pdf.internal.pageSize.getWidth(); // Largeur A4 (210mm)
        const pageHeight = pdf.internal.pageSize.getHeight(); // Hauteur A4 (297mm)
        ...
        // Ajout des pages supplémentaires tant qu'il reste du contenu
        while (heightLeft > 0) {
        ...
        if (data.success) { 
            toast.success("Document envoyé et enregistré !", { id: 'pdf-gen' }); 
...
const AdminDashboard = () => {
  ...
  // ✅ OPTION VERRE CORRIGÉE (Correspond au prix)
  ...
  const handleSaveMontage = async (e: React.FormEvent) => { 
      ...
      const basePayload = { reference: newRef, frame: newFrame, description: newDesc, category: newCategory, glassType: newGlassType, urgency: newUrgency, diamondCutType: newDiamondCutType, engravingCount: newEngravingCount, shapeChange: newShapeChange, createdBy: "Admin" };
  ...
  const openEditDialog = (m: Montage) => { setEditingId(m._id); setNewRef(m.reference||""); setNewFrame(m.frame||""); setNewCategory(m.category||"Cerclé"); setNewGlassType(m.glassType||[]); setNewUrgency(m.urgency||"Standard"); setNewDiamondCutType(m.diamondCutType||"Standard"); setNewEngravingCount(m.engravingCount||0); setNewShapeChange(m.shapeChange||false); setNewDesc(m.description||""); setNewClient(m.userId); setIsDialogOpen(true); };
                                ...
                                {m.photoUrl ? (
                                    <Button 
                                        ...
                                        onClick={async (e) => {
                                            ...
                                            toast.loading("Téléchargement de la photo...", { id: 'load-photo' });
                                            ...
                                            try {
                                                ...
                                                if (data.success && data.montage.photoUrl) {
                                                    // 3. On ouvre la modale avec la vraie image
                                                    setSelectedPhotoUrl(data.montage.photoUrl);
                                                    toast.dismiss('load-photo');
                                                } else {
                                                    toast.error("Image introuvable ou corrompue", { id: 'load-photo' });
                                                }
                                            } catch (err) {
                                                toast.error("Erreur de connexion serveur", { id: 'load-photo' });
                                            }
                                        }}
                                    >
                                    ...
                                    <>
                                        {/* Cas sans photo : On affiche la caméra pour uploader */}
                                        <input 
                                            type="file" 
                                            accept="image/*" 
                                            style={{ display: 'none' }} 
                                            ...
                                            onChange={(e) => { 
                                                if (e.target.files && e.target.files[0]) 
                                                    handlePhotoUpload(m._id, e.target.files[0]); 
                                            }} 
                                        />
                                        <Button 
                                            variant="outline" 
                                            ...
                                            onClick={(e) => { 
                                                e.stopPropagation();
                                                fileInputRefs.current[m._id]?.click();
                                            }}
                                        >
