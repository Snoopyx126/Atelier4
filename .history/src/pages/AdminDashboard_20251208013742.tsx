...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose }) => {
    ...
    const calculateTotal = (m: Montage) => { /* ... */ return { total: 0, surcharge: 0 }; };
    const getMontagePriceDetails = (m: Montage) => { /* ... */ return { total: 0, details: [] as string[] }; };
  ...
  const fetchClients = async () => {
    try {
        ...
        if (data.success && Array.isArray(data.users)) { 
            // La simulation d'adresse ici est critique pour l'affichage de la facture
