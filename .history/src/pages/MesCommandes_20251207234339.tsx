...
const MesCommandes = () => {
  ...
  const [shapeChange, setShapeChange] = useState(false); // ✅ NOUVEAU
    ...
    try {
        const res = await fetch("https://atelier4.vercel.app/api/montages", {
            ...
            body: JSON.stringify({ 
                ...
                glassType, urgency, diamondCutType, engravingCount, shapeChange // ✅ ENVOI
            })
        });
        ...
        if (data.success) {
            ...
            setGlassType([]); setUrgency("Standard"); setDiamondCutType("Standard"); setEngravingCount(0); setShapeChange(false); // ✅ RESET
  ...
  const filteredMontages = montages.filter(m => {
    ...
    if (!term) return true;
    return [m.reference, m.frame, m.description, m.category, m.statut, m.urgency, m.diamondCutType, dateStr].some(field => normalize(field).includes(term));
  });
                ...
                <div className="space-y-3 p-4 border rounded-lg bg-gray-50">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <label htmlFor="shapeChangeClient" className="font-semibold text-gray-700">Changement de Forme (+{SHAPE_CHANGE_COST}€ HT)</label>
                        </div>
                    </div>
                </div>
