...
import { LayoutDashboard, Glasses, Users, Package, AlertCircle, CheckCircle2, Trash2, Mail, FileText, Calendar, PlusCircle, Pencil, Search, Phone, Receipt, Printer, Send, Clock } from "lucide-react";
...
const URGENCY_COSTS: Record<string, number> = { 'Urgent -48H': 5.00, 'Urgent -24H': 10.00, 'Urgent -3H': 20.00, 'Standard': 0.00 };

const SHAPE_CHANGE_COST = 10.00;
const ENGRAVING_UNIT_COST = 12.00; 

// --- INFO SOCIÉTÉ (VOTRE ADRESSE) ---
const FACTURE_INFO = {
    name: "L'Atelier des Arts",
    address: "178 Avenue Daumesnil",
    zipCity: "75012 Paris",
    siret: "[Votre SIRET]",
    email: "contact@atelierdesarts.com",
    tvaRate: 0.20 // 20%
};


// --- COMPOSANT MODAL DE FACTURATION (FINAL) ---
...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose }) => {
    ...
    // Fonction unique pour détailler la ligne de coût
    const getMontagePriceDetails = (m: Montage) => {
        ...
        // 2. Urgence
        const urgencyPrice = URGENCY_COSTS[m.urgency || 'Standard'] || 0;
        if (urgencyPrice > 0) {
            total += urgencyPrice;
            details.push(`Urgence (${urgencyPrice.toFixed(2)}€)`);
        }
        ...
        // 6. Changement de Forme
        if (m.shapeChange) {
            total += SHAPE_CHANGE_COST;
    ...
    const invoiceNumber = `FCT-${currentYear}${currentMonth + 1}-${client._id.substring(0, 4)}`.toUpperCase();

    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl bg-white p-8 print:p-0">
                <div className="invoice-body p-4 print:p-0">
                    ...
                    <div className="flex justify-between items-start mb-10 border-b pb-4 border-gray-300">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-sm text-gray-600">À l'attention de: {client.nomSociete}</p>
                            <p className="text-sm text-gray-600">Email: {client.email}</p>
                            <p className="text-sm text-gray-600">SIRET: {client.siret || 'N/A'}</p>
                            <p className="text-xs mt-2">Date d'édition: {today.toLocaleDateString('fr-FR')}</p>
                        </div>
                    </div>

                    <h3 className="font-bold text-lg mb-3 border-b pb-2">Prestations du mois de {monthName}</h3>
