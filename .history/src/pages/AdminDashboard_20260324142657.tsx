...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose, onInvoicePublished }) => {
    ...
    const invoiceNumber = `FCT-${currentYear}${today.getMonth() + 1}${today.getDate()}-${client._id.substring(0, 4)}-${Math.floor(Math.random() * 1000)}`.toUpperCase();
    ...
    const handlePublishAndSend = async () => {
        ...
        try {
            ...
            const payload = { 
                ...
                pdfUrl: '#', 
                sendEmail: true, 
                // 🚨 MAGIE ICI : Si plus de 20 dossiers, on envoie 'null' pour ne pas faire exploser Vercel
