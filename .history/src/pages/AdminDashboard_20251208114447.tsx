        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <style>{`
                @media print {
                    ...
                    .invoice-wrapper { visibility: visible; position: absolute; top: 0; left: 0; width: 100%; padding: 0 !important; margin: 0 !important; box-shadow: none; }
                    .print-hidden { display: none !important; }
                    .DialogContent { max-width: 100% !important; width: 100% !important; max-height: none !important; overflow: visible !important; }
                }
            `}</style>
            <div className="invoice-wrapper"> 
                <DialogContent className="DialogContent max-w-4xl bg-white p-8 print:p-0 max-h-[90vh] overflow-y-auto">
                    <div className="print-hidden">
                        <DialogHeader><DialogTitle className="text-3xl font-bold text-gray-900 flex items-center gap-2"><Receipt className="w-6 h-6" /> Facture Provisoire ({monthlyMontages.length} montages)</DialogTitle></DialogHeader>
                    </div>

                    <div id="invoice-content" className="invoice-body p-4 print:p-0">
                        
                        {/* EN-TÊTE FACTURE */}
                        <div className="flex justify-between items-start mb-10 border-b pb-4 border-gray-300">
                            {/* VOTRE SOCIÉTÉ */}
                            <div>
                                <h1 className="text-3xl font-extrabold text-black">{FACTURE_INFO.name}</h1>
                                ...
                                <p className="text-sm text-gray-600">Email: {FACTURE_INFO.email}</p>
                            </div>
                            {/* INFOS CLIENT */}
                            <div className="text-right">
                                <h2 className="text-2xl font-bold text-gray-700 mb-2">Facture N° {invoiceNumber}</h2>
                                <p className="text-sm font-semibold">{client.nomSociete}</p>
                                {client.address ? (
                                    <>
                                    ...
                                    <p className="text-sm text-red-500 font-medium">Adresse Client: N/A</p>
                                )}
                                <p className="text-sm text-gray-600 mt-1">SIRET: {client.siret || 'N/A'}</p>
                                <p className="text-xs mt-2">Date d'édition: {today.toLocaleDateString('fr-FR')}</p>
                            </div>
                        </div>

                        <h3 className="font-bold text-lg mb-3 border-b pb-2">Prestations Facturées</h3>

                        <div className="mt-6 border rounded-lg overflow-hidden">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-gray-100 text-xs uppercase text-gray-700">
                                    <tr>
                                        <th scope="col" className="px-6 py-3">RÉF. CLIENT & MONTURE</th>
                                        <th scope="col" className="px-6 py-3">DÉTAIL DES OPTIONS</th>
                        ...
                        {monthlyMontages.length === 0 && (<p className="text-center py-4 text-gray-500">Aucun montage "Terminé" facturable.</p>)}

                        {/* PIED DE FACTURE (TOTAUX) */}
                        <div className="flex justify-end mt-8">
                            <div className="w-full max-w-sm">
                                <div className="p-4 bg-gray-100 rounded-t-lg space-y-1">
                                    <div className="flex justify-between font-medium"><span>Total HT</span><span>{totalHT.toFixed(2)} €</span></div>
                                    <div className="flex justify-between text-sm"><span>Taux TVA</span><span>{FACTURE_INFO.tvaRate * 100}%</span></div>
                                    <div className="flex justify-between text-sm"><span>Montant TVA</span><span>{tva.toFixed(2)} €</span></div>
                                </div>
                                <div className="p-4 bg-black text-white rounded-b-lg flex justify-between text-xl font-extrabold">
                                    <span>TOTAL TTC</span>
...
interface ClientInvoicesModalProps { client: Client | null; invoices: any[]; isOpen: boolean; onClose: () => void; }
                ...
                {invoices.length === 0 ? (
                    ...
                    <div className="space-y-3 pt-4">
                        {invoices.map(invoice => (
                            <Card key={invoice.id} className="flex justify-between items-center p-3">
                                <div>
...
const AdminDashboard = () => {
  ...
  // États pour la MODALE LISTE DES FACTURES (Nouveaux)
  const [isClientInvoicesModalOpen, setIsClientInvoicesModalOpen] = useState(false);
  const [currentClientInvoices, setCurrentClientInvoices] = useState<any[]>([]); 
  ...
  const fetchClients = async () => {
    try {
        ...
        if (data.success && Array.isArray(data.users)) { 
            ...
            // Set first client as default for new montage form
  ...
  // ✅ Fonction de simulation de récupération des factures pour l'Admin (LISTE)
    const fetchInvoicesAdmin = async (clientId: string) => {
        // ⚠️ SIMULATION: Pour l'instant, on simule des factures liées au client pour l'affichage
        const facturesSimulees = [
            { id: 'FCT-202511-001', userId: clients[0]?._id, clientName: clients[0]?.nomSociete || 'Client 1', totalTTC: 124.80, dateEmission: '2025-11-01', pdfUrl: '#' },
            { id: 'FCT-202511-002', userId: clients[1]?._id, clientName: clients[1]?.nomSociete || 'Client 2', totalTTC: 300.00, dateEmission: '2025-11-15', pdfUrl: '#' },
            { id: 'FCT-202512-003', userId: clients[0]?._id, clientName: clients[0]?.nomSociete || 'Client 1', totalTTC: 239.90, dateEmission: '2025-12-01', pdfUrl: '#' },
        ];
        // Filtrer les factures simulées pour le client sélectionné
        return facturesSimulees.filter(f => f.userId === clientId);
    };

    // ✅ Fonction pour ouvrir la modale des factures client (au clic sur le client)
    const openClientInvoices = async (client: Client) => {
        const clientInvoices = await fetchInvoicesAdmin(client._id);
  ...
  const groupedByMonthAndShop = filteredMontages.reduce((monthGroups: Record<string, Record<string, Montage[]>>, montage: Montage) => {
    ...
    if (!monthGroups[monthYear][shop]) {
        monthGroups[monthYear][shop] = [];
    }
      ...
      <div className="flex-grow pt-24 pb-10 px-6 container mx-auto max-w-7xl">
        ...
        <Tabs defaultValue="atelier" className="space-y-6">
            <TabsList className="bg-white p-1 shadow-sm border h-auto w-full md:w-auto">
                ...
                <TabsTrigger value="clients" className="px-6 py-2"><Users className="w-4 h-4 mr-2" /> Fiches Clients ({filteredClients.length})</TabsTrigger> {/* Titre mis à jour */}
            </TabsList>
                        ...
                        {filteredClients.length === 0 ? <p className="text-center text-gray-500 italic">Aucun client trouvé.</p> : (
                            filteredClients.map(c => (
                                <div key={c._id} 
                                     ...
                                     onClick={() => openClientInvoices(c)} // ✅ NOUVELLE FONCTION DE CLIC
                                >
                                    <div>
                                    ...
                                    <Button size="sm" variant="outline" className="text-green-600 border-green-200 hover:bg-green-50">
                                        <Receipt className="w-4 h-4 mr-2" /> Voir les Factures
                                    </Button>
                                </div>
                            ))
                        )}
