...
import { LayoutDashboard, Glasses, Users, Package, AlertCircle, CheckCircle2, Trash2, Mail, FileText, Calendar, PlusCircle, Pencil, Search, Phone, Receipt } from "lucide-react";

// Interfaces Mises à Jour (pour la base de données)
interface Montage {
  _id: string;
  clientName: string;
  reference?: string;
...
const InvoiceModal: React.FC<InvoiceProps> = ({ client, montages, isOpen, onClose }) => {
    ...
    // Filtre les montages Terminés pour le mois en cours
    ...
    // Simplification du calcul des coûts (à titre d'exemple)
    const baseCost = 15; // Coût de base du montage
    const urgencyCost = { 'Urgent -48H': 5, 'Urgent -24H': 10, 'Urgent -3H': 20, 'Standard': 0 };
    const diamondCutCost = { 'Facette Lisse': 15, 'Diamond Ice': 25, 'Facette Twinkle': 35, 'Standard': 0 };

    const calculateTotal = (m: Montage) => {
        let total = baseCost;
        total += urgencyCost[m.urgency as keyof typeof urgencyCost] || 0;
        total += diamondCutCost[m.diamondCutType as keyof typeof diamondCutCost] || 0;
        total += (m.engravingCount || 0) * 5; // 5€ par gravure
        // Ajoutez ici la logique de prix complexe
        return total;
    };
            ...
            <DialogContent className="max-w-4xl bg-white p-8">
                <DialogHeader>
                    <DialogTitle className="text-3xl font-bold text-gray-900 flex items-center gap-2">
                        <Receipt className="w-6 h-6" /> Facture Provisoire ({monthName})
                    </DialogTitle>
                </DialogHeader>

                <div className="mt-4">
                    <h3 className="font-bold text-xl mb-3">{client.nomSociete}</h3>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 text-xs uppercase text-gray-700">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">MONTAGE / FINITION</th>
                                <th scope="col" className="px-6 py-3 text-right">COÛT (HT)</th>
                            </tr>
                        </thead>
                        <tbody>
                            {monthlyMontages.map(m => (
                                <tr key={m._id} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        {m.category} | {m.diamondCutType} | {m.urgency !== 'Standard' && m.urgency}
                                    </td>
                ...
                {monthlyMontages.length === 0 && (
                    <p className="text-center py-4 text-gray-500">Aucun montage "Terminé" facturable ce mois-ci.</p>
                )}
...
const AdminDashboard = () => {
  ...
  // [ ... (Déclaration des autres états de formulaire et Constantes de choix inchangées) ... ]
  ...
  // --- NOUVELLE FONCTION : DÉCLENCHER LA FACTURATION ---
  ...
  // [ ... (Logique des fonctions de Fetch, OpenDialog, SaveMontage, HandleStatusChange, etc. inchangée) ... ]
  // [ ... (Logique de filtrage et de KPIs inchangée) ... ]

  // Remplacez ces lignes par vos vrais états/logiques de l'AdminDashboard :
                        ...
                        <form onSubmit={handleSaveMontage} className="space-y-4 pt-4">
                            ...
                            <div className="space-y-3 p-4 border rounded-lg bg-gray-50">
                                ...
                                <div className="flex gap-6">
                                    {GLASS_OPTIONS.map(opt => (<div key={opt} className="flex items-center space-x-2"><Checkbox id={`glass-${opt}`} checked={newGlassType.includes(opt)} onCheckedChange={(checked) => handleGlassTypeChange(opt, checked as boolean)} /><label htmlFor={`glass-${opt}`}>{opt}</label></div>))}
                                </div>
                            </div>
                        ...
                        {Object.keys(groupedByShop).length === 0 ? (<div className="text-center py-20 text-gray-400">Aucun montage trouvé pour cette recherche.</div>) : (
                            <Accordion type="single" collapsible className="space-y-4">
                                {Object.entries(groupedByShop).sort().map(([shopName, items]) => {
                                    const client = clients.find(c => c.nomSociete === shopName) || { _id: '', nomSociete: shopName }; // Trouver l'objet client
                                    return (
                                            ...
                                            <AccordionTrigger className="hover:no-underline py-4">
                                                <div className="flex items-center gap-4 w-full pr-4">
                                                    ...
                                                    <Button 
                                                        variant="outline" 
                                                        size="sm" 
                                                        className="ml-auto flex items-center gap-1 text-xs bg-black text-white hover:bg-gray-800"
                                                        onClick={(e) => {
                                                            e.stopPropagation(); // Empêche l'accordéon de s'ouvrir
                                                            handleGenerateInvoice(client as Client, items);
                                                        }}
                                                    >
