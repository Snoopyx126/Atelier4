...
import { Search, FileText, ShoppingCart, Receipt, Calendar, PlusCircle, Image as ImageIcon, X, UserCog, Store, RefreshCw, Loader2 } from "lucide-react";
...
interface Montage { _id:string;reference:string;frame:string;description:string;category:string;glassType?:string[];urgency?:string;diamondCutType?:string;engravingCount?:number;shapeChange?:boolean;statut:string;dateReception:string;photoUrl?:string;createdBy?:string;clientName?:string;userId?:string; }
...
export default function MesCommandes(){
  ...
  const fm=montages.filter(m=>normalize(m.reference+m.frame).includes(normalize(search)));
  ...
  const renderCard=(m:Montage)=>{
    const tier=isManager?(clientsList.find((c:any)=>c._id===m.userId)?.pricingTier||1):(user?.pricingTier||1);
    ...
    return(
      <div key={m._id} className="bg-white border border-[#EDE8DF] rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 hover:shadow-sm transition-all">
        <div className="flex-1 min-w-0">
          ...
          {isManager&&m.createdBy?.includes("Manager")&&<div className="inline-flex items-center gap-1.5 text-[10px] text-blue-600 bg-blue-50 border border-blue-100 rounded-full px-2.5 py-0.5 mb-2"><UserCog className="w-2.5 h-2.5"/>{m.createdBy}</div>}
          <div className="flex flex-wrap gap-1.5 mb-2">
            ...
            {m.shapeChange&&<span className="text-[10px] rounded-full px-2 py-0.5 bg-amber-50 border border-amber-100 text-amber-600">Changement forme</span>}
          </div>
        ...
        {m.photoUrl&&<button className={S.btnO+" h-9 w-9 p-0"} onClick={()=>setPhotoUrl(m.photoUrl!)}><ImageIcon className="w-3.5 h-3.5 text-[#C9A96E]"/></button>}
      </div>
    );
  };
      ...
      <div className="flex-grow pt-24 pb-12 px-6 container mx-auto max-w-5xl">
        ...
        {tab==='commandes'&&(
          <div className="space-y-6">
            ...
            {/* Dossiers */}
            {Object.keys(grouped).length===0?<div className="text-center py-16 text-gray-300 text-sm">Aucune commande trouvée.</div>:(
