...
// --- STYLES D'IMPRESSION (FIX DU PROBLÈME 2) ---
const PRINT_STYLES = `
    @media print {
        body > div:not(.invoice-wrapper) {
            display: none;
        }
        .invoice-wrapper {
            visibility: visible;
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            ...
            <div className="invoice-wrapper"> 
                {/* ✅ FIX SCROLL : Ajout de max-h-[90vh] et overflow-y-auto */}
...
const AdminDashboard = () => {
  ...
  const fetchClients = async () => {
    try {
        ...
        const data = await res.json();
        if (data.success) { 
            // Simulation de l'ajout d'adresse/ville pour la facture (à connecter à votre API SIRET)
            const clientsWithAddress = data.users.map((c: Client) => ({
                ...c,
                address: "12 Rue de la Monture",
                zipCity: "75001 Optique Ville" 
            }));
