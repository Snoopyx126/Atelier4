    ...
    hiddenDiv.innerHTML = `
        <div style="font-family: sans-serif; color: #000;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 40px;">
                <div><h1 style="font-size: 30px; font-weight: bold; margin-bottom: 10px;">FACTURE</h1><p style="color: #666;">N° ${facture.invoiceNumber}</p><p style="color: #999;">Date: ${new Date(facture.dateEmission).toLocaleDateString('fr-FR')}</p></div>
                <div style="text-align: right;"><h2 style="font-size: 18px; font-weight: bold;">L'Atelier des Arts</h2><p style="color: #666; font-size: 12px;">178 Avenue Daumesnil, 75012 Paris</p><p style="color: #666; font-size: 12px;">SIRET: 98095501700010</p></div>
            </div>
            <div style="border-top: 2px solid #eee; padding-top: 20px; margin-bottom: 30px;"><h3 style="font-size: 12px; font-weight: bold; color: #999; text-transform: uppercase;">Facturé à</h3><p style="font-size: 18px; font-weight: bold;">${user.nomSociete}</p><p style="color: #666;">${user.siret}</p></div>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
                ...
                <tbody>${facture.invoiceData.map((item: any) => `<tr style="border-bottom: 1px solid #eee;"><td style="padding: 10px 0; vertical-align: top;">${item.reference || '-'}</td><td style="padding: 10px 0; font-size: 12px; color: #666;">${item.details.join('<br/>')}</td><td style="padding: 10px 0; text-align: right; font-weight: bold;">${item.price.toFixed(2)} €</td></tr>`).join('')}</tbody>
            </table>
            <div style="text-align: right;"><p style="margin: 5px 0;">Total HT: ${facture.totalHT?.toFixed(2)} €</p><p style="margin: 5px 0;">TVA (20%): ${(facture.totalTTC - (facture.totalHT || 0)).toFixed(2)} €</p><h2 style="font-size: 24px; font-weight: bold; margin-top: 10px;">Net à payer: ${facture.totalTTC.toFixed(2)} €</h2></div>
        </div>
    `;
