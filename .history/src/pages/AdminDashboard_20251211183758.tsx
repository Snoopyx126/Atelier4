...
import { Textarea } from "@/components/ui/textarea"; // Import ajouté
...
const AdminDashboard = () => {
  ...
  const [newDesc, setNewDesc] = useState(""); // ✅ ÉTAT POUR LA DESCRIPTION/COMMENTAIRE
  ...
  // ✅ NOUVEL ÉTAT DE FILTRE PAR STATUT (null = tout afficher)
  ...
  // ✅ LOGIQUE DE SAUVEGARDE CORRIGÉE (Ne modifie plus l'userId en PUT et inclut la description)
  const handleSaveMontage = async (e: React.FormEvent) => { 
      ...
      const basePayload = { 
          reference: newRef, 
          frame: newFrame, 
          description: newDesc, // ✅ ENVOI DE LA DESCRIPTION
  ...
  const openCreateDialog = () => { 
    ...
    setNewDesc(""); // ✅ Reset la description pour la création
  ...
  const openEditDialog = (m: Montage) => { 
    ...
    setNewDesc(m.description||""); // ✅ Charge la description existante
    setNewClient(m.userId); // ✅ CORRECTION BUG : Initialise l'ID client pour la modale
    setIsDialogOpen(true); 
  };
  ...
  // AFFICHAGE DU RÉSUMÉ : Masquer la gravure si elle est à 0 ou non définie
  ...
  // ✅ FILTRAGE : Applique le filtre de recherche ET le filtre de statut
  const filteredMontages = montages.filter(m => {
      const searchMatch = normalize(m.reference + m.clientName).includes(normalize(searchTerm));
      
      let statusMatch = true;
      if (statusFilter === 'En production') {
          // Si on filtre sur "En production", on inclut En cours OU Reçu
          statusMatch = (m.statut === 'En cours' || m.statut === 'Reçu');
      } else if (statusFilter) {
          // Si un autre statut spécifique est sélectionné (e.g., 'En attente', 'Terminé')
  ...
  // ✅ REGROUPEMENT CORRIGÉ : Utilise une clé fiable basée sur la liste 'clients'
      ...
      <div className="flex-grow pt-24 pb-10 px-6 container mx-auto max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex gap-3">
                ...
                {/* ✅ MODALE AVEC FORMULAIRE CORRIGÉ (Changement de Forme & Commentaire) */}
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                  <DialogContent className="max-w-3xl bg-white">
                      ...
                      <form onSubmit={handleSaveMontage} className="space-y-4 pt-4">
                          ...
                          <div className="grid grid-cols-3 gap-4">
                              ...
                              <div>
                                  ...
                                  <Select onValueChange={setNewDiamondCutType} value={newDiamondCutType}>
                                      <SelectTrigger className="bg-white"><SelectValue/></SelectTrigger>
                                      <SelectContent className="bg-white">{DIAMONDCUT_OPTIONS.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                                  </Select>
                              </div>
                              <div>
                                  <Label>Gravure</Label>
                                  <Input type="number" value={newEngravingCount} onChange={e=>setNewEngravingCount(parseInt(e.target.value))} className="bg-white"/>
                              </div>
                          </div>
                          
                          {/* BLOC OPTIONS VERRES & CHANGEMENT DE FORME */}
                          ...
                          <div>
                              <Label htmlFor="description">Commentaire / Description (visible par le client)</Label>
                              <Textarea 
                                  id="description" 
                                  value={newDesc} 
                                  onChange={e => setNewDesc(e.target.value)} 
                                  placeholder="Indiquer ici toute information spécifique nécessaire au montage ou à la facturation." 
                                  className="bg-white"
                              />
                          </div>
        ...
        {/* ✅ CARTES FILTRABLES : On ajoute le onClick et le style de sélection */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {/* CARTE "À TRAITER" (En attente) */}
            <Card 
                className={`shadow-sm border-l-4 border-l-red-500 cursor-pointer transition-shadow ${statusFilter === 'En attente' ? 'ring-2 ring-red-500' : 'hover:shadow-lg'}`}
                onClick={() => setStatusFilter(statusFilter === 'En attente' ? null : 'En attente')}
            >
            ...
            {/* CARTE "EN PRODUCTION" (En cours ou Reçu) */}
            <Card 
                className={`shadow-sm border-l-4 border-l-blue-500 cursor-pointer transition-shadow ${statusFilter === 'En production' ? 'ring-2 ring-blue-500' : 'hover:shadow-lg'}`}
                onClick={() => setStatusFilter(statusFilter === 'En production' ? null : 'En production')}
            >
                <CardHeader><CardTitle>En Production</CardTitle></CardHeader>
                <CardContent><div className="text-2xl font-bold">{inProgressCount}</div></CardContent>
            </Card>
            
            {/* CARTE "CLIENTS" (Reste inchangée) */}
        ...
        {statusFilter && (
            <div className="mb-4">
                <Button variant="outline" size="sm" onClick={() => setStatusFilter(null)} className="gap-2">
                    Filtre actif : {statusFilter} (Cliquez pour annuler)
                    <X className="w-4 h-4" />
                </Button>
            </div>
        )}
                    ...
                    return (
                        <div key={m._id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-gray-50 rounded-lg border border-gray-100 gap-4 transition-all hover:bg-white hover:shadow-md">
                            <div className="flex-1">
                                ...
                                {/* AFFICHAGE DESCRIPTION/COMMENTAIRE SOUS LES BADGES */}
                                {m.description && m.description.length > 0 && (
                                    <p className="text-xs text-gray-600 mt-1 mb-2 italic border-l-2 border-gray-200 pl-2 max-w-full overflow-hidden whitespace-normal">
                                        Note: {m.description}
                                    </p>
                                )}
