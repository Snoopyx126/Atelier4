...
const InvoiceModal=({client,montages,isOpen,onClose,onPublished,onError,autoPublish,progress}:{client:Client;montages:Montage[];isOpen:boolean;onClose:()=>void;onPublished:(f:FactureData)=>void;onError?:(clientName:string,err:any)=>void;autoPublish?:boolean;progress?:{current:number;total:number}})=>{
  ...
  const publish=async()=>{
    ...
    try{
      let el=document.getElementById('inv-content');
      if(!el){
        // Le Dialog vient peut-être tout juste de se (re)monter (1er client d'un lot) :
        // on laisse jusqu'à 1s au DOM pour se peindre avant d'abandonner.
        for(let i=0;i<10&&!el;i++){
          await new Promise(r=>setTimeout(r,100));
          el=document.getElementById('inv-content');
        }
      }
      if(!el)throw new Error("Élément #inv-content introuvable dans le DOM (la facture n'a pas eu le temps de s'afficher).");
      let canvas;
      try{
        canvas=await html2canvas(el,{scale:1,useCORS:true,scrollY:0});
      }catch(e:any){throw new Error("Échec html2canvas (capture visuelle de la facture) : "+(e?.message||e));}
      let pdf,w,ip;
      try{
        pdf=new jsPDF({orientation:'portrait',unit:'mm',format:'a4',compress:true});
        w=pdf.internal.pageSize.getWidth();
        ip=pdf.getImageProperties(canvas.toDataURL('image/jpeg',0.3));
        pdf.addImage(canvas.toDataURL('image/jpeg',0.3),'JPEG',0,0,w,(ip.height*w)/ip.width);
      }catch(e:any){throw new Error("Échec génération jsPDF : "+(e?.message||e));}
      ...
      let res;
      try{
        res=await authFetch(`${getBase()}/api/factures`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      }catch(e:any){throw new Error("Échec réseau vers /api/factures : "+(e?.message||e));}
      let d;
      try{
        d=await res.json();
      }catch(e:any){throw new Error(`Réponse serveur invalide (statut ${res.status}, pas du JSON).`);}
      if(!res.ok&&!d?.success){throw new Error(`Serveur (HTTP ${res.status}) : ${d?.message||d?.error||'erreur inconnue'}`);}
  ...
  useEffect(()=>{
    if(autoPublish&&isOpen&&montages.length>0){
      const t=setTimeout(()=>publish(),50); // laisse le Dialog peindre avant de capturer
      ...
      <div className="flex-grow pt-24 pb-12 px-4 sm:px-6 container mx-auto max-w-7xl">
        ...
        <Dialog open={shopOpen} onOpenChange={setShopOpen}>
          <DialogContent className="bg-white max-w-lg rounded-2xl">
            ...
            <div className="py-4 space-y-2 max-h-[60vh] overflow-y-auto">
              {clients.filter(c=>c.role==='user').sort((a,b)=>a.nomSociete.localeCompare(b.nomSociete,'fr',{sensitivity:'base'})).map(shop=>(
        ...
        <Dialog open={dlgOpen} onOpenChange={setDlgOpen}>
          <DialogContent className="max-w-2xl bg-white rounded-2xl">
            ...
            <form onSubmit={saveMontage} className="space-y-5 pt-2">
              <div><Label className={S.label}>Client</Label><Select onValueChange={setNClient} value={nClient}><SelectTrigger className={S.inp+" w-full h-10"}><SelectValue/></SelectTrigger><SelectContent className="bg-white rounded-xl">{[...clients].sort((a,b)=>a.nomSociete.localeCompare(b.nomSociete,'fr',{sensitivity:'base'})).map(c=><SelectItem key={c._id} value={c._id}>{c.nomSociete}</SelectItem>)}</SelectContent></Select></div>
        ...
        <MonthSelectModal
          ...
          months={allMonthKeys.map(k=>{
            ...
            const shopItemsArr=Object.values(shops) as Montage[][];
            const montageCount:number=shopItemsArr.reduce((s:number,items:Montage[])=>s+items.length,0);
