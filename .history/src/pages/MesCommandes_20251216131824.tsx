...
const MontageCard = ({ m, tier, onPhoto, onEdit, onDelete, isManager, fileInputRefs, handleUpload }: any) => {
    ...
    // Droits : Manager modifie tout, Client modifie/supprime seulement si "En attente/Reçu"
...
const MesCommandes = () => {
  ...
  // --- FILTRAGE ET REGROUPEMENT (AVEC CORRECTION "INCONNU") ---
  ...
  const grouped = filtered.reduce((acc: any, m) => {
      ...
      if (userData?.role === 'manager') {
          // 1. Essayer le nom enregistré dans le dossier
          shopKey = m.clientName || "Magasin Inconnu";
          
          // 2. CORRECTION : Si c'est "Inconnu", on cherche dans la liste des magasins assignés grâce à l'ID
          if (shopKey === "Magasin Inconnu" && userData.assignedShops) {
              const foundShop = userData.assignedShops.find((s:any) => s._id === m.userId);
              if (foundShop) shopKey = foundShop.nomSociete;
          }
      }
