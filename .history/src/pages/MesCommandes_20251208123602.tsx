...
const MesCommandes = () => {
  ...
  // ✅ VRAIE RÉCUPÉRATION DES FACTURES PUBLIÉES PAR L'ADMIN
    ...
    try {
        ...
        if (data.success && Array.isArray(data.factures)) {
            setFactures(data.factures.map((f: any) => ({
                ...
                // Assurez-vous que montagesReferences est un tableau avant de faire join
                montageReference: f.montagesReferences && Array.isArray(f.montagesReferences) ? f.montagesReferences.join(', ') : 'N/A', 
        ...
        setFactures([]);
        console.error("Erreur de connexion API pour factures client:", error);
    }
  };
  ...
  const getStatusColor = (statut: string) => { /* ... */ return 'bg-gray-500'; };
                    ...
                    <div className="space-y-3">
                        {factures.map(f => (
                            <Card key={f.invoiceNumber} className="p-4 flex justify-between items-center shadow-sm">
                                <div>
                                    <p className="font-bold text-lg text-gray-900">{f.invoiceNumber}</p>
                                    <p className="text-sm text-gray-500">Émise le {new Date(f.dateEmission).toLocaleDateString('fr-FR')}</p>
                                    <p className="text-xs text-gray-400">Réf. Montages: {f.montageReference}</p>
                                </div>
