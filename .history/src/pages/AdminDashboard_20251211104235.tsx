...
const AdminDashboard = () => {
  ...
  // Ligne 200 (Début de la fonction handleSaveMontage)
const handleSaveMontage = async (e: React.FormEvent) => { 
    ...
    // Payload de base contenant toutes les données modifiables
    const basePayload = { 
        reference: newRef, 
        frame: newFrame, 
        description: newDesc, 
    ...
    // ✅ CORRECTION 2 : Ajouter userId SEULEMENT en mode POST (Création)
    const payload = method === "POST" 
        ? { ...basePayload, userId: newClient }
        : basePayload; // En mode PUT, on ne touche pas à l'ID client
    
    try { 
        const res = await fetch(url, { 
            method, 
            headers: { "Content-Type": "application/json" }, 
            body: JSON.stringify(payload) // Utilisation du payload conditionnel
        }); 
  ...
  // ✅ CORRECTION 1 : Initialiser le client du montage
