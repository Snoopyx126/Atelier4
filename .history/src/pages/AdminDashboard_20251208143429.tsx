...
import { LayoutDashboard, Glasses, Users, Package, AlertCircle, CheckCircle2, Trash2, Mail, FileText, Calendar, PlusCircle, Pencil, Search, Phone, Receipt, Printer, Send, Loader2, CreditCard, Camera, Image as ImageIcon, X } from "lucide-react";
    ...
    const handlePublishAndSend = async () => {
        ...
        try {
            ...
            const invoiceDetailsSnapshot = monthlyMontages.map(m => ({
                reference: m.reference, details: getMontagePriceDetails(m).details, price: getMontagePriceDetails(m).total
            }));
            const payload = {
                userId: client._id, clientName: client.nomSociete, invoiceNumber: invoiceNumber,
                totalHT: parseFloat(totalHT.toFixed(2)), totalTTC: parseFloat(totalTTC.toFixed(2)),
                montagesReferences: montagesReferences, dateEmission: new Date().toISOString(),
                invoiceData: invoiceDetailsSnapshot, pdfUrl: '#', sendEmail: true, pdfBase64: pdfBase64 
            };
            ...
            if (data.success) {
                ...
                onClose();
            } else { toast.error(`Erreur: ${data.message}`, { id: 'pub-load' }); }
        } catch (error) { console.error(error); toast.error("Erreur technique.", { id: 'pub-load' }); } finally { setIsPublishing(false); }
    };
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            ...
            <div className="invoice-wrapper"> 
                <DialogContent className="DialogContent max-w-4xl bg-white p-0 overflow-hidden flex flex-col max-h-[95vh]">
                    <DialogHeader className="p-6 border-b print-hidden"><DialogTitle className="text-3xl font-bold text-gray-900">Facturation Mensuelle</DialogTitle></DialogHeader>
                    <div className="overflow-y-auto p-6 bg-gray-50 flex-grow">
                        <div id="invoice-content" className="p-10 bg-white text-black border shadow-sm mx-auto max-w-[210mm] min-h-[297mm] flex flex-col justify-between"> 
                            <div>
                                <div className="flex justify-between items-start mb-10"><div><h1 className="text-4xl font-extrabold text-gray-900 mb-2">FACTURE</h1><p className="text-gray-500 font-medium">N° {invoiceNumber}</p><p className="text-sm text-gray-400">Date: {today.toLocaleDateString('fr-FR')}</p></div><div className="text-right"><h2 className="text-xl font-bold text-gray-800">{FACTURE_INFO.name}</h2><p className="text-sm text-gray-600">{FACTURE_INFO.address}</p><p className="text-sm text-gray-600">{FACTURE_INFO.zipCity}</p><p className="text-sm text-gray-600">SIRET: {FACTURE_INFO.siret}</p></div></div>
                                <div className="border-t-2 border-gray-100 py-6 mb-8"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Facturé à</h3><p className="text-xl font-bold text-gray-900">{client.nomSociete}</p><p className="text-gray-600">{client.address}</p><p className="text-gray-600">{client.zipCity}</p><p className="text-gray-600 text-sm mt-1">SIRET: {client.siret}</p></div>
                                <table className="w-full mb-8"><thead><tr className="border-b-2 border-gray-800"><th className="text-left py-3 font-bold text-gray-900">Description</th><th className="text-left py-3 font-bold text-gray-900">Détails</th><th className="text-right py-3 font-bold text-gray-900">Prix HT</th></tr></thead><tbody className="divide-y divide-gray-200">{monthlyMontages.map((m) => { const { total, details } = getMontagePriceDetails(m); return (<tr key={m._id}><td className="py-4 align-top font-medium text-gray-800">{m.reference}</td><td className="py-4 align-top text-sm text-gray-600">{details.map((d, i) => <div key={i}>{d}</div>)}</td><td className="py-4 align-top text-right font-bold text-gray-900">{total.toFixed(2)} €</td></tr>); })}</tbody></table>
                            </div>
                            <div>
                                <div className="flex justify-between items-end mb-8"><div className="bg-gray-50 border border-gray-200 p-4 rounded-lg w-1/2 mr-8"><h4 className="font-bold text-sm text-gray-900 mb-2">COORDONNÉES BANCAIRES</h4><div className="text-xs text-gray-600 space-y-1"><p>Banque : <strong>L'ATELIER DES ARTS</strong></p><p>IBAN :</p><p className="font-mono text-sm font-bold text-gray-800 tracking-wider">FR76 1820 6002 0065 1045 3419 297</p><p className="text-[10px] text-gray-400 mt-1">BIC : VOTRE_CODE_BIC_ICI</p></div></div><div className="w-1/3 space-y-3"><div className="flex justify-between text-gray-600"><span className="font-medium">Total HT</span><span>{totalHT.toFixed(2)} €</span></div><div className="flex justify-between text-gray-600"><span className="font-medium">TVA (20%)</span><span>{tva.toFixed(2)} €</span></div><div className="flex justify-between text-2xl font-extrabold text-gray-900 border-t-2 border-gray-900 pt-3"><span>Net à payer</span><span>{totalTTC.toFixed(2)} €</span></div></div></div>
                                <div className="pt-4 border-t border-gray-100 text-center text-[10px] text-gray-400 leading-tight"><p>Conditions de règlement : Paiement à réception. Aucun escompte pour paiement anticipé.</p><p>Retard de paiement : indemnité forfaitaire 40€ + pénalités de retard (3x taux légal).</p><p className="mt-2 font-medium">{FACTURE_INFO.name} - SIRET {FACTURE_INFO.siret}</p></div>
                            </div>
                        </div>
                    </div>
                    <div className="p-4 border-t bg-gray-50 flex justify-end gap-3 print-hidden flex-shrink-0 z-50 relative"><Button variant="outline" onClick={onClose} disabled={isPublishing}>Annuler</Button><Button onClick={handlePublishAndSend} disabled={isPublishing || monthlyMontages.length === 0} className="bg-black text-white hover:bg-gray-800">{isPublishing ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Send className="w-4 h-4 mr-2" />}Valider, Envoyer & Sauvegarder</Button></div>
                </DialogContent>
            </div>
        </Dialog>
    );
...
const ClientInvoicesModal: React.FC<ClientInvoicesModalProps> = ({ client, invoices, isOpen, onClose, onDelete, onPaymentUpdate }) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-3xl bg-white p-6 max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle className="text-2xl font-bold">Factures : {client.nomSociete}</DialogTitle></DialogHeader>{invoices.length === 0 ? <div className="text-center py-10 text-gray-500">Aucune facture.</div> : (<div className="space-y-3 pt-4">{invoices.map(inv => { const paid = inv.amountPaid || 0; const remaining = inv.totalTTC - paid; return (<Card key={inv.id} className="flex flex-col md:flex-row justify-between items-center p-4 gap-4"><div className="flex-1"><div className="flex items-center gap-2 mb-1"><p className="font-semibold">{inv.invoiceNumber}</p>{getStatusBadge(inv.paymentStatus, remaining)}</div><p className="text-xs text-gray-500">{new Date(inv.dateEmission).toLocaleDateString()}</p></div><div className="flex items-center gap-4"><div className="text-right"><div className="font-bold text-lg">{inv.totalTTC.toFixed(2)} €</div>{paid > 0 && paid < inv.totalTTC && <div className="text-xs text-gray-400">Déjà payé: {paid.toFixed(2)} €</div>}</div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => openPayment(inv)} title="Enregistrer un paiement"><CreditCard className="w-4 h-4" /></Button><Button size="sm" variant="destructive" onClick={() => { if(confirm("Supprimer ?")) onDelete(inv.id); }} title="Supprimer"><Trash2 className="w-4 h-4" /></Button></div></div></Card>); })}</div>)}</DialogContent>
            {payInvoice && (<Dialog open={!!payInvoice} onOpenChange={() => setPayInvoice(null)}><DialogContent className="max-w-sm bg-white"><DialogHeader><DialogTitle>Enregistrer un paiement</DialogTitle></DialogHeader><div className="space-y-4 py-4"><p className="text-sm text-gray-600">Facture {payInvoice.invoiceNumber} <br/> Total: <strong>{payInvoice.totalTTC.toFixed(2)} €</strong></p><div className="space-y-2"><Label>Montant total réglé à ce jour (€)</Label><Input type="number" step="0.01" value={amountInput} onChange={e => setAmountInput(parseFloat(e.target.value))} /><div className="flex gap-2 text-xs"><Button size="sm" variant="outline" onClick={() => setAmountInput(payInvoice.totalTTC)}>Tout régler</Button><Button size="sm" variant="outline" onClick={() => setAmountInput(0)}>Réinitialiser</Button></div></div><Button className="w-full bg-green-600 hover:bg-green-700 text-white" onClick={submitPayment}>Valider le paiement</Button></div></DialogContent></Dialog>)}
        </Dialog>
    );
};
...
const AdminDashboard = () => {
  ...
  const handlePhotoUpload = async (montageId: string, file: File) => {
      ...
      toast.loading("Envoi de la photo...", { id: 'photo-upload' });
      const baseUrl = window.location.hostname === "localhost" ? "http://localhost:3000" : "https://atelier4.vercel.app";
      ...
      try {
          ...
          const data = await res.json();
          if (data.success) {
              toast.success("Photo ajoutée avec succès !", { id: 'photo-upload' });
              // Mise à jour locale de l'état des montages pour afficher le nouveau bouton image
              setMontages(prev => prev.map(m => m._id === montageId ? { ...m, photoUrl: data.montage.photoUrl } : m));
          } else {
              toast.error(`Erreur upload: ${data.message}`, { id: 'photo-upload' });
          }
      } catch (error) {
          toast.error("Erreur de connexion lors de l'upload.", { id: 'photo-upload' });
      }
  };


  const handleSaveMontage = async (e: React.FormEvent) => { e.preventDefault(); setIsSubmitting(true); const baseUrl = window.location.hostname === "localhost" ? "http://localhost:3000" : "https://atelier4.vercel.app"; const method = editingId ? "PUT" : "POST"; const url = editingId ? `${baseUrl}/api/montages/${editingId}` : `${baseUrl}/api/montages`; try { const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: newClient, reference: newRef, frame: newFrame, description: newDesc, category: newCategory, glassType: newGlassType, urgency: newUrgency, diamondCutType: newDiamondCutType, engravingCount: newEngravingCount, shapeChange: newShapeChange }) }); const data = await res.json(); if (data.success) { toast.success(editingId ? "Modifié !" : "Créé !"); setIsDialogOpen(false); fetchMontages(); } } catch (error) { toast.error("Erreur API"); } finally { setIsSubmitting(false); } };
...
<div key={m._id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-gray-50 rounded-lg border border-gray-100 gap-4 transition-all hover:bg-white hover:shadow-md">
    <div className="flex-1"><div className="mb-2"><span className="font-bold text-xl text-gray-900">{m.reference}</span><span className="text-gray-400 mx-2">|</span><span className="font-semibold text-gray-700">{m.frame}</span></div><div className="flex flex-wrap items-center gap-2 mb-2"><Badge variant="outline" className="bg-white">{m.category}</Badge>{renderMontageDetails(m)}</div></div>
    <div className="flex items-center gap-3 w-full sm:w-auto">
        ...
        <Button variant="outline" size="icon" onClick={() => handleDelete(m._id)} className="bg-white"><Trash2 className="w-4 h-4 text-red-600" /></Button>
            ...
            <TabsContent value="clients"><Card className="shadow-md border-0"><CardHeader className="bg-white border-b"><CardTitle>Fiches Clients & Facturation</CardTitle></CardHeader><CardContent className="p-6">{filteredClients.map(c => (<div key={c._id} className="p-4 border-b last:border-0 flex justify-between items-center bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer" onClick={() => openClientInvoices(c)}><div><p className="font-bold text-lg">{c.nomSociete}</p><p className="text-sm text-gray-500">{c.email}</p></div><Button size="sm" variant="outline" className="text-green-600 border-green-200 hover:bg-green-50 bg-white"><Receipt className="w-4 h-4 mr-2" /> Voir les Factures</Button></div>))}</CardContent></Card></TabsContent>
        </Tabs>
