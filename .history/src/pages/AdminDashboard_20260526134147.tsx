          ...
          <DialogContent className="max-w-2xl bg-white rounded-2xl">
            ...
            <form onSubmit={saveMontage} className="space-y-5 pt-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div><Label className={S.label}>Réf.</Label><Input value={nRef} onChange={e=>setNRef(e.target.value)} required<div><Label className={S.label}>Client</Label><Select onValueChange={setNClient} value={nClient}><SelectTrigger className={S.inp+" w-full h-10"}><SelectValue/></SelectTrigger><SelectContent className="bg-white rounded-xl">{[...clients].sort((a,b)=>a.nomSociete.localeCompare(b.nomSociete,'fr')).map(c=><SelectItem key={c._id} value={c._id}>{c.nomSociete}</SelectItem>)}</SelectContent></Select></div> className={S.inp+" h-10"}/></div>
