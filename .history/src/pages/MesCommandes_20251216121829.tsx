...
// --- HELPER FUNCTIONS (Sorties du composant pour être accessibles partout) ---

// 1. Calcul des détails (Badges)
...
// 2. Rendu de la carte (Utilise renderMontageDetails)
const renderMontageCard = (
    ...
    // Modification : Manager (Toujours) OU Client (Si autorisé)
    const canEdit = isManager || (m.statut === 'En attente' || m.statut === 'Reçu');
    
    // Suppression : UNIQUEMENT Client (Si autorisé). Manager n'a JAMAIS le droit.
    ...
    return (
        <div key={m._id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-white rounded-lg border border-gray-100 gap-4 transition-all hover:shadow-md ${getStatusColor(m.statut)}`}>
            <div className="flex-1">
                ...
                <div className="mb-2 flex gap-2 items-center">
                     ...
                     {m.createdBy && m.createdBy.includes("Manager") && <span className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded border border-blue-100">{m.createdBy}</span>}
                </div>
                 ...
                 {m.photoUrl ? (
                    <Button variant="outline" size="icon" className="text-blue-600 border-blue-200 hover:bg-blue-50 bg-white" onClick={async () => {
                        ...
                        try {
                            ...
                            } else { toast.error("Introuvable", { id: 'load-photo' }); }
                        } catch (e) { toast.error("Erreur", { id: 'load-photo' }); }
                    }}><ImageIcon className="w-4 h-4" /></Button>
                ) : (
                    <><input type="file" accept="image/*" style={{ display: 'none' }} ref={el => fileInputRefs.current[m._id] = el} onChange={(e) => { if (e.target.files && e.target.files[0]) handlePhotoUpload(m._id, e.target.files[0]); }} /><Button variant="outline" size="icon" className="text-gray-600 border-gray-300 hover:bg-gray-50 bg-white" onClick={() => fileInputRefs.current[m._id]?.click()}><Camera className="w-4 h-4" /></Button></>
                )}
...
const MesCommandes = () => {
  ...
  const groupedByMonthAndShop = filteredMontages.reduce((acc: any, m) => {
      ...
      const shopKey = userData?.role === 'manager' 
          ? (m.clientName || "Magasin Inconnu") 
          : "Mes Montages";

      if (!acc[monthName]) acc[monthName] = {};
      if (!acc[monthName][shopKey]) acc[monthName][shopKey] = [];
      
      acc[monthName][shopKey].push(m);
      return acc;
  }, {});
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-grow pt-24 pb-10 px-4 md:px-8 container mx-auto max-w-7xl">
            ...
            <div>
                ...
                <p className="text-gray-500">
                    {userData.role === 'manager' 
                        ? "Vue Manager : Suivi global de vos boutiques." 
        ...
        <div className="mb-6 relative"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" /><Input className="pl-10 bg-white shadow-sm h-12" placeholder="Rechercher une référence, une monture, un magasin..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
            ...
            {['encours', 'historique'].map(tab => (
                <TabsContent key={tab} value={tab}>
                    <Card className="shadow-md border-0"><CardContent className="p-6 bg-gray-50/50 min-h-[400px]">
                        {Object.keys(groupedByMonthAndShop).length === 0 ? <div className="text-center py-20 text-gray-400">Aucun dossier trouvé.</div> : (
                            <Accordion type="multiple" className="space-y-4">
                                {Object.entries(groupedByMonthAndShop).map(([monthName, shopGroups]: any) => {
                                    
                                    const visibleShopGroups = Object.entries(shopGroups).reduce((acc: any, [shopName, items]: any) => {
                                        const filteredItems = (items as Montage[]).filter(m => {
                                            const isFinished = m.statut === 'Terminé' || m.statut === 'Expédié';
                                            return tab === 'historique' ? isFinished : !isFinished;
                                        });
                                        if (filteredItems.length > 0) acc[shopName] = filteredItems;
                                        return acc;
                                    }, {});

                                    if (Object.keys(visibleShopGroups).length === 0) return null;
                                    ...
                                    return (
                                        <AccordionItem key={monthName} value={monthName} className="bg-white border rounded-lg shadow-xl px-4">
                                            ...
                                            <AccordionContent className="pt-4 pb-6 space-y-4">
                                                {Object.entries(visibleShopGroups).map(([shopName, items]: any) => (
                                                    <div key={shopName} className="bg-gray-50/50 border rounded-lg p-3">
                                                        {userData.role === 'manager' && (
                                                            <div className="flex items-center gap-2 mb-3 border-b pb-2 px-1">
                                                                <Store className="w-4 h-4 text-blue-600"/>
                                                                <h3 className="font-bold text-gray-800 text-md">{shopName}</h3>
