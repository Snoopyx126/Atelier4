...
app.put("/api/users/:id", async (req, res) => {
    ...
    try {
        ...
        // Si l'utilisateur veut changer des infos sensibles (email/mdp), on vérifie son mot de passe actuel
        if ((email !== user.email || newPassword) && !currentPassword) {
             return res.status(400).json({ success: false, message: "Mot de passe actuel requis pour ces modifications." });
        }
