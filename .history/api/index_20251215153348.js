...
// 2. LISTE OPTIMISÉE (Empêche le crash erreur 500)
// 2. LISTE OPTIMISÉE (Version Intelligente avec Aggregation)
app.get("/api/montages", async (req, res) => {
    try {
        const { role, userId, managerId } = req.query;
        let query = {};
        ...
        if (role === 'manager' && managerId) {
            const manager = await User.findById(managerId);
            // Conversion des IDs en format Mongoose pour l'agrégation
        ...
        // --- 2. Agrégation (La magie opère ici) ---
        const montages = await Montage.aggregate([
            { $match: query }, // On filtre (Admin voit tout, Client voit le sien)
            { $sort: { dateReception: -1 } }, // On trie par date
            {
                $addFields: {
                    // On crée un champ 'hasPhoto' qui est VRAI si photoUrl existe et n'est pas vide
                    hasPhoto: { 
                        $cond: { 
                            if: { $and: [ { $ne: ["$photoUrl", null] }, { $ne: ["$photoUrl", ""] } ] }, 
                            then: true, 
                            else: false 
                        }
                    }
            ...
            { $project: { photoUrl: 0 } } // IMPORTANT : On retire la photo lourde du résultat final
        ]);

        // --- 3. Formatage pour le Frontend ---
        const optimizedMontages = montages.map(m => ({
            ...m,
            // Si hasPhoto est vrai, on met le marqueur, sinon null (ce qui affichera la Caméra)
