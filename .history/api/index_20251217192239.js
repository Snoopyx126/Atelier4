...
app.get("/api/fix-glass-names", async (req, res) => {
    try {
        const montages = await Montage.find({ glassType: { $exists: true } });
        let updatedCount = 0;
        ...
        for (let m of montages) {
            ...
            const newGlassType = m.glassType.map(g => {
                ...
                // Si le texte contient '4', 'saison' et 'degrad', on le transforme en nom propre
        ...
        res.json({ success: true, message: `${updatedCount} dossiers ont été réparés.` });
