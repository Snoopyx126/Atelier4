...
// ✅ MODÈLE FACTURE MIS À JOUR (PAIEMENT)
const factureSchema = new mongoose.Schema({
    ...
    amountPaid: { type: Number, default: 0 }, // Montant déjà réglé
        ...
        if (sendEmail && pdfBase64) {
            ...
            if (user && user.email) {
                ...
                await resend.emails.send({
                    ...
                    html: `<p>Bonjour ${user.nomSociete},</p><p>Veuillez trouver ci-joint votre facture mensuelle <strong>${invoiceNumber}</strong>.</p><p>Cordialement,<br>L'équipe de l'Atelier des Arts</p>`,
                    attachments: [{ filename: `Facture_${invoiceNumber}.pdf`, content: pdfBuffer }],
                });
            }
        }
...
// ✅ 11. PUT FACTURE (Mise à jour Paiement)
app.put("/api/factures/:id", async (req, res) => {
    ...
    try {
        ...
        if (amountPaid !== undefined) {
            facture.amountPaid = amountPaid;
        }

        // Calcul automatique du statut
        if (facture.amountPaid >= facture.totalTTC - 0.1) { // Tolérance de 10 centimes
            facture.paymentStatus = 'Payé';
        } else if (facture.amountPaid > 0) {
            facture.paymentStatus = 'Partiellement payé';
        } else {
            facture.paymentStatus = 'Non payé';
        }
