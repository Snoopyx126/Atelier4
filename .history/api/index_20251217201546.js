        ...
        for (let m of montages) {
            ...
            const newGlassType = m.glassType.map(g => {
                ...
                if (lower.includes('4') && lower.includes('saison') && lower.includes('degrad')) {
                    if (g !== "Verre Dégradé 4 saisons") { changed = true; return "Verre Dégradé 4 saisons"; }
                }
                return g;
            });
            if (changed) { await Montage.findByIdAndUpdate(m._id, { glassType: newGlassType }); updatedCount++; }
        }
        res.json({ success: true, message: `${updatedCount} dossiers réparés.` });
