...
// 3. MOT DE PASSE OUBLIÉ (Version Démo via EmailJS)
app.post("/api/forgot-password", async (req, res) => {
    ...
    try {
        const user = await User.findOne({ email });
        if (!user) {
            // Pour la démo, on le dit si l'email n'existe pas
        ...
        // 3. ⚠️ RENVOYER LE MOT DE PASSE AU FRONTEND (Pour envoi via EmailJS)
        // Ceci est la partie "insécurisée" spécifique à cette méthode de démo
        res.json({ 
            success: true, 
            message: "Mot de passe généré.", 
            tempPassword: tempPassword // On le renvoie pour que le navigateur l'envoie par mail
        });
