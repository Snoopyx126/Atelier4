...
app.get("/api/fix-glass-names", async (req, res) => {
    try {
        // On récupère tous les montages qui ont des verres
        const montages = await Montage.find({ glassType: { $exists: true, $not: { $size: 0 } } });
        let updatedCount = 0;
        
        for (let m of montages) {
            let changed = false;
            // On vérifie chaque verre dans la liste
            const newGlassType = m.glassType.map(g => {
                const lower = g.toLowerCase();
                // Si le texte contient '4', 'saison' et 'degrad' mais n'est pas le bon nom
                if (lower.includes('4') && lower.includes('saison') && lower.includes('degrad')) {
            ...
            if (changed) {
                await Montage.findByIdAndUpdate(m._id, { glassType: newGlassType });
                updatedCount++;
            }
        }
        res.json({ 
            success: true, 
            message: `${updatedCount} dossiers ont été réparés avec succès.` 
        });
    } catch (e) { 
