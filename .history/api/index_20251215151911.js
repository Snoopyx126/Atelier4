...
app.get("/api/montages", async (req, res) => {
    try {
        ...
        // Logique de filtrage (Manager vs User vs Admin)
        if (role === 'manager' && managerId) {
            const manager = await User.findById(managerId);
            query = { userId: { $in: manager.assignedShops } };
        } else if (role !== 'admin' && userId) {
        ...
        // .lean() permet une lecture plus rapide
        const montages = await Montage.find(query).sort({ dateReception: -1 }).lean();

        // On remplace la lourde image Base64 par un simple marqueur pour la liste
        const optimizedMontages = montages.map(m => ({
            ...m,
            photoUrl: m.photoUrl ? "DISPONIBLE" : null 
        }));
