...
app.post("/api/forgot-password", async (req, res) => {
    ...
    try {
        const user = await User.findOne({ email });
        if (!user) {
            // Sécurité : ne pas dire si l'email existe ou pas
        ...
        await resend.emails.send({
            from: "onboarding@resend.dev",
            to: email,
            subject: "Atelier des Arts - Mot de passe oublié",
            html: `
                <h1>Mot de passe oublié ?</h1>
                <p>Voici votre nouveau mot de passe temporaire :</p>
                <h2 style="background: #eee; padding: 10px; display: inline-block;">${tempPassword}</h2>
                <p>Connectez-vous et modifiez-le immédiatement dans votre profil.</p>
            `
        });
