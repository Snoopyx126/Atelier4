...
const userSchema = new mongoose.Schema({
  ...
  phone: { type: String },
  address: { type: String }, // ✅ AJOUT SCHÉMA ADRESSE
  zipCity: { type: String }, // ✅ AJOUT SCHÉMA CODE POSTAL/VILLE
...
// 2. LOGIN (Mise à jour pour inclure les champs d'adresse dans la réponse)
...
// 4. UPDATE USER (Mise à jour pour accepter les champs d'adresse)
app.put("/api/users/:id", async (req, res) => {
    ...
    try {
        ...
        if (address !== undefined) user.address = address; // ✅ MISE À JOUR
        if (zipCity !== undefined) user.zipCity = zipCity; // ✅ MISE À JOUR
...
// 5. MONTAGES - GET USERS (Reste inchangé)
app.get("/api/users", async (req, res) => {
    await connectDB();
    try {
        const users = await User.find({ role: { $ne: 'admin' } }).select('-password'); 
        // Note: MongoDB renverra address/zipCity s'ils existent sur l'objet user.
        res.json({ success: true, users });
    } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});

// 5. MONTAGES - PUT (Correction pour le statut, reste inchangé)
