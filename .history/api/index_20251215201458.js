    ...
    try {
        ...
        if (role === 'manager' && managerId) {
            if (mongoose.Types.ObjectId.isValid(managerId)) {
                ...
                // On récupère les IDs valides des magasins
        ...
        // --- RÉCUPÉRATION ---
        // .find() est plus souple que aggregate
        // .select('-photoUrl') empêche le chargement des images lourdes (Anti-Crash)
        // .lean() accélère la lecture
        const montages = await Montage.find(query)
                                      .select('-photoUrl') 
                                      .sort({ dateReception: -1 })
                                      .lean();

        console.log(`✅ Montages trouvés : ${montages.length}`); // Regardez les logs Vercel pour voir ce nombre

        // --- FORMATAGE ---
        const optimizedMontages = montages.map(m => ({
            ...m,
            // Pour l'instant, on met null pour forcer l'affichage de la liste sans bug.
            // Cela affichera l'icône CAMÉRA 📷 partout.
            photoUrl: null 
        }));
        ...
        // En cas d'erreur, on renvoie une liste vide pour ne pas casser l'interface
