...
// 1. CRÉATION (Avec logs pour débogage)
app.get("/api/montages", async (req, res) => {
    try {
        ...
        // ✅ LE SECRET EST ICI : .select('-photoUrl')
        // Cela dit à MongoDB : "Donne-moi tout SAUF la photoUrl"
        // Le chargement passera de 100 Mo à 0.1 Mo -> Très rapide !
        const montages = await Montage.find(query)
                                      .select('-photoUrl') 
                                      .sort({ dateReception: -1 })
                                      .lean();

        // On triche un peu : on dit au Frontend que la photo est "DISPONIBLE"
        // (On suppose qu'il y en a une, on vérifiera au clic)
        const optimizedMontages = montages.map(m => ({
            ...m,
            // On met un marqueur pour que le bouton "Œil" apparaisse potentiellement
            // Note : Sans la vraie donnée, on ne peut pas être sûr à 100% qu'il y a une photo,
            // mais on permet au frontend d'essayer de la charger.
            photoUrl: "DISPONIBLE" 
        }));
        
        res.json({ success: true, montages: optimizedMontages });
    } catch (e) { 
        console.error("Erreur liste montages:", e);
        res.status(500).json({ success: false }); 
    }
});

// 2. LISTE OPTIMISÉE (Version Infaillible "Longueur")
app.get("/api/montages", async (req, res) => {
    try {
        const { role, userId, managerId } = req.query;
        let query = {};
        
        // --- 1. Construction du filtre (Adapté pour Aggregation) ---
        if (role === 'manager' && managerId) {
            const manager = await User.findById(managerId);
        ...
        // --- 2. Agrégation avec vérification de la TAILLE de l'image ---
        const montages = await Montage.aggregate([
            { $match: query }, 
            { $sort: { dateReception: -1 } },
            {
                $addFields: {
                    // On calcule la longueur du texte de la photo (0 si vide)
                    // Cela évite les bugs de photos "fantômes"
                    photoLength: { $strLenCP: { $ifNull: ["$photoUrl", ""] } }
                }
            },
            {
                $addFields: {
                    // C'est une vraie photo SEULEMENT si le code fait plus de 100 caractères
                    hasRealPhoto: { $gt: ["$photoLength", 100] }
                }
            },
            { $project: { photoUrl: 0, photoLength: 0 } } // On retire le lourd, on garde le léger
        ]);

        // --- 3. Formatage pour le Frontend ---
        const optimizedMontages = montages.map(m => ({
            ...m,
            // Si hasRealPhoto est vrai, on met le marqueur. 
            // Sinon on met NULL, ce qui fera apparaître la Caméra.
            photoUrl: m.hasRealPhoto ? "DISPONIBLE" : null 
        }));
...
// 3. DÉTAIL UNIQUE (C'est ici qu'on récupère la vraie photo au clic)
