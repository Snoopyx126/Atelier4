...
// 2. LISTE OPTIMISÉE (Version Chirurgicale "Regex")
app.get("/api/montages", async (req, res) => {
    try {
        const { role, userId, managerId } = req.query;
        let query = {};
        ...
        const montages = await Montage.aggregate([
            ...
            {
                $addFields: {
                    // On force la conversion en string et on vérifie si ça commence par "data:image"
                    // C'est la signature OBLIGATOIRE d'une image valide
                    isTrueImage: { 
                        $regexMatch: { 
        ...
        const optimizedMontages = montages.map(m => ({
            ...m,
            // Si c'est une vraie image, on met le marqueur. Sinon NULL (Caméra).
            photoUrl: m.isTrueImage ? "DISPONIBLE" : null 
        }));
