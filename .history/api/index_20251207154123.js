...
const montageSchema = new mongoose.Schema({
  ...
  clientName: String,
  description: String,
  statut: { type: String, default: 'En attente' }, // Par défaut : En attente
...
app.post("/api/login", async (req, res) => {
    ...
    try {
        ...
        // Log pour débuggage Vercel
        console.log("LOGIN TEST - Email:", user.email, "Role trouvé:", user.role);
...
// 5. GESTION DES MONTAGES (MODIFIÉ)
...
// Créer un montage (Client ou Admin)
app.post("/api/montages", async (req, res) => {
    await connectDB();
    const { userId, description } = req.body; // 'statut' retiré ici pour forcer la logique interne
    try {
        const user = await User.findById(userId);
        if (!user) return res.status(404).json({ message: "Client introuvable" });
        
        // ✅ Modification : Statut par défaut "En attente" pour les nouvelles créations
...
// ✅ NOUVELLE ROUTE : Mettre à jour un montage (Pour valider la réception / changer statut)
