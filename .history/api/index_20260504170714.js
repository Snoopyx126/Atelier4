...
const createPennylaneInvoice = async (customerId, invoiceNumber, montagesData, totalHT, deadline) => {
    ...
    const invoiceLines = montagesData.map(m => {
        ...
        return {
            label: m.reference ? `${m.reference} — ${m.details.join(', ')}` : "Montage",
            quantity: 1,
    ...
    // ✅ STRUCTURE CRUCIALE : Pour draft: false, tout doit être dans l'objet 'customer_invoice'
    const payload = {
        customer_invoice: {
            ...
            invoice_lines_attributes: invoiceLines, // ⚠️ Notez le '_attributes' parfois requis en v2
            draft: false,
            // Optionnel : on peut forcer la devise si besoin
            currency: "EUR"
        }
    };
    ...
    // Si l'erreur 'NotAnyOf' persiste avec invoice_lines_attributes, 
    // essayez de remettre 'invoice_lines' mais gardez bien l'enveloppe 'customer_invoice'
    if (!res.ok) {
        // Tentative de repli automatique si invoice_lines_attributes n'est pas reconnu
        if (JSON.stringify(data).includes("invoice_lines_attributes")) {
            payload.customer_invoice.invoice_lines = invoiceLines;
            delete payload.customer_invoice.invoice_lines_attributes;
            const retryRes = await fetch(`${PENNYLANE_API}/customer_invoices`, {
                method: 'POST',
                headers,
                body: JSON.stringify(payload)
            });
            const retryData = await retryRes.json();
            if (!retryRes.ok) throw new Error(`Erreur Pennylane: ${JSON.stringify(retryData)}`);
            return retryData.customer_invoice;
        }
