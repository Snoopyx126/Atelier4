...
app.get("/api/montages", async (req, res) => {
    try {
        ...
        let matchStage = {}; // Filtre vide par défaut (Admin voit tout)

        // --- 1. FILTRAGE SÉCURISÉ (Conversion ObjectId obligatoire pour Aggrégation) ---
        if (role === 'manager' && managerId) {
            if (mongoose.Types.ObjectId.isValid(managerId)) {
                const manager = await User.findById(managerId);
                const shopIds = (manager?.assignedShops || [])
                    .filter(id => mongoose.Types.ObjectId.isValid(id))
                    .map(id => new mongoose.Types.ObjectId(id)); // ⚠️ Conversion CRUCIALE
                matchStage = { userId: { $in: shopIds } };
            }
        } else if (role !== 'admin' && userId) {
            if (mongoose.Types.ObjectId.isValid(userId)) {
                matchStage = { userId: new mongoose.Types.ObjectId(userId) }; // ⚠️ Conversion CRUCIALE
            } else {
                return res.json({ success: true, montages: [] });
            }
        }

        // --- 2. AGGREGATION (Scan rapide) ---
        const montages = await Montage.aggregate([
            { $match: matchStage }, // On applique le filtre
            { $sort: { dateReception: -1 } },
            {
                $addFields: {
                    // On vérifie si la photo existe en regardant sa taille (plus de 100 caractères)
                    // $ifNull protège contre les bugs si le champ n'existe pas
                    hasPhoto: { 
                        $gt: [{ $strLenCP: { $ifNull: [{ $toString: "$photoUrl" }, ""] } }, 100] 
                    }
                }
            },
            {
                $project: {
                    photoUrl: 0, // 🛑 ON SUPPRIME LA PHOTO LOURDE (Anti-Crash)
                    __v: 0
                }
            }
        ]);
        ...
        const optimizedMontages = montages.map(m => ({
            ...m,
            // Si hasPhoto est vrai, on met le code "DISPONIBLE" -> Affiche l'Œil 👁️
            // Sinon on met null -> Affiche la Caméra 📷
        ...
        res.json({ success: true, montages: [] }); // Sécurité
    }
});
