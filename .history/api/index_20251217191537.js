...
// --- ROUTE DE RÉPARATION ULTIME ---
app.get("/api/fix-glass-names", async (req, res) => {
    try {
        // 1. Correction pour le texte inversé exact
        const res1 = await Montage.updateMany(
            { glassType: "Verre 4 Dégradé saisons" }, 
            { $set: { "glassType.$[elem]": "Verre Dégradé 4 saisons" } },
            { arrayFilters: [{ "elem": "Verre 4 Dégradé saisons" }] }
        );

        // 2. Correction de sécurité si le texte est simplement "4 Dégradé saisons"
        const res2 = await Montage.updateMany(
            { glassType: "4 Dégradé saisons" }, 
            { $set: { "glassType.$[elem]": "Verre Dégradé 4 saisons" } },
            { arrayFilters: [{ "elem": "4 Dégradé saisons" }] }
        );

        res.json({ 
            success: true, 
            message: `Nettoyage fini. ${res1.modifiedCount + res2.modifiedCount} dossiers corrigés.` 
        });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
