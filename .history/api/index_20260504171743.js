...
// --- 2. BASE DE DONNÉES ---
// Pattern officiel MongoDB pour serverless : on cache la connexion sur global
// pour qu'elle persiste entre les invocations Vercel (sinon chaque cold start
// recrée une connexion → sature le pool M0 et fait timeout).
// Ref : https://www.mongodb.com/developer/products/atlas/serverless-mongodb-atlas/
...
const connectDB = async () => {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(process.env.MONGODB_URI, {
      serverSelectionTimeoutMS: 5000,    // échec rapide si Atlas est lent
      socketTimeoutMS: 30000,
      maxPoolSize: 5,                     // M0 free tier = pool minimal pour pas saturer
      minPoolSize: 0,
      bufferCommands: false,              // les requêtes échouent vite si pas connecté (au lieu de buffer 10s)
    }).then((m) => {
      console.log("✅ Connecté à MongoDB");
      return m;
    }).catch((err) => {
      cached.promise = null;              // pour pouvoir réessayer au prochain appel
      throw err;
    });
  }
...
const createPennylaneInvoice = async (customerId, invoiceNumber, montagesData, totalHT, deadline) => {
    ...
    // On revient à la détection de prix qui marchait et au nom de clé correct
    const invoiceLines = montagesData.map(m => {
        // On récupère le prix (m.total est celui envoyé par votre Frontend)
        const unitPrice = m.total || m.price || 0;
        
        // On reconstruit le label proprement
        const detailsText = (m.details && Array.isArray(m.details)) ? m.details.join(', ') : '';

        return {
            label: m.reference ? `${m.reference} — ${detailsText}` : "Montage",
            quantity: 1,
            raw_currency_unit_price: Number(unitPrice).toFixed(2),
            vat_rate: 'FR_200',
            unit: 'piece'
        };
    });
    ...
    // ✅ STRUCTURE DIRECTE (SANS ENVELOPPE) - Validée par l'erreur précédente
    const payload = {
        customer_id: customerId,
        date: today.toISOString().split('T')[0],
        deadline: deadlineDate.toISOString().split('T')[0],
        invoice_number: invoiceNumber,
        invoice_lines: invoiceLines, // On reste sur invoice_lines
        draft: false
    };
...
const getOrCreatePennylaneCustomer = async (user) => {
    ...
    // 1. Recherche par Référence Externe
    const filterRef = JSON.stringify([{ field: "external_reference", operator: "eq", value: externalRef }]);
    const searchRes = await fetch(
        `${PENNYLANE_API}/company_customers?filter=${encodeURIComponent(filterRef)}`,
        { headers }
    );
    const searchData = await searchRes.json();
    let foundCustomers = searchData.customers || searchData.company_customers || (Array.isArray(searchData) ? searchData : []);

    // 2. Si non trouvé par Ref, on cherche par Email
    if (foundCustomers.length === 0 && user.email) {
        const filterEmail = JSON.stringify([{ field: "emails", operator: "contains", value: user.email }]);
        const searchEmailRes = await fetch(
            `${PENNYLANE_API}/company_customers?filter=${encodeURIComponent(filterEmail)}`,
            { headers }
        );
        const searchEmailData = await searchEmailRes.json();
        foundCustomers = searchEmailData.customers || searchEmailData.company_customers || (Array.isArray(searchEmailData) ? searchEmailData : []);
    }
    ...
    // 3. Création si le client n'existe nulle part
    ...
    if (!customerId) {
        throw new Error(`Erreur client Pennylane: ${JSON.stringify(createData)}`);
    }
    return customerId;
};
